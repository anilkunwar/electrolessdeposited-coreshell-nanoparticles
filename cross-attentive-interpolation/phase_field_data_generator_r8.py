...
* **PKL filename now includes L₀ (nm), core fraction (fc), shell ratio (rs)**
"""
...
bc_type = st.sidebar.selectbox("BC in boundaries except top", ["Neumann (zero flux)", "Dirichlet (fixed values)"])
...
shell_thickness_frac = st.sidebar.slider("Δr / r_core", 0.005, 0.4, 0.2, 0.005)
...
L0_nm = st.sidebar.number_input("L₀ (nm)", 1.0, 1e6, 20.0)   # keep in nm for filename
L0 = L0_nm * 1e-9                                             # nm → m
...
def run_simulation(c_bulk_val):
    ...
    save_data = {
        ...
        "parameters": {
            ...
            "L0_m": float(L0), "L0_nm": float(L0_nm), "tau0_s": float(tau0),
        },
        "coords_nd": coords,
        "snapshots": snapshots,
        "diagnostics": diags,
