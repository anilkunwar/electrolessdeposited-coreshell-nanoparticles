...
COLORMAP_OPTIONS = [
    'viridis', 'plasma', 'inferno', 'magma', 'cividis', 'turbo', 'hot', 'afmhot', 'jet', 'rainbow',
    'hsv', 'RdBu', 'RdYlBu', 'Spectral', 'coolwarm', 'bwr', 'seismic', 'BrBG', 'tab10', 'tab20',
    'Set1', 'Set2', 'Set3', 'Accent', 'Dark2', 'Paired', 'Pastel1', 'Pastel2', 'Greys', 'PuRd',
    'GnBu', 'YlGnBu', 'YlGn', 'Blues', 'BuGn', 'Greens', 'OrRd', 'PuBu', 'PuBuGn', 'RdPu', 'Reds',
    'YlOrBr', 'YlOrRd', 'Oranges', 'Purples', 'bone', 'copper', 'pink', 'spring', 'summer', 'autumn',
    'winter', 'cool', 'Wistia', 'terrain', 'ocean', 'gist_earth', 'gist_stern', 'gist_rainbow',
    'gist_ncar', 'nipy_spectral', 'twilight', 'twilight_shifted', 'binary', 'gray', 'flag', 'prism'
]  # Expanded to 50+ colormaps
# =============================================
...
class HybridWeightVisualizer:
    ...
    def create_parameter_radar_charts(self, sources_data, target_params, param_sigmas):
        ...
        for pkey, pname in zip(param_keys, param_names):
            ...
            # Build figure
            fig = go.Figure()
            # Customdata: [param_value, raw_weight]
            fig.add_trace(go.Scatterpolar(
                ...
                customdata=list(zip(p_vals, l0_weights_raw)),
                hovertemplate='<b>α (L0) Weight</b>: %{r:.3f} (norm)<br>Raw: %{customdata[1]:.3f}<br>%{theta:.1f}° (Param: %{customdata[0]:.2f})<extra></extra>'
            ))
            fig.add_trace(go.Scatterpolar(
                ...
                customdata=list(zip(p_vals, fc_weights_raw)),
                hovertemplate='<b>β_fc Weight</b>: %{r:.3f} (norm)<br>Raw: %{customdata[1]:.3f}<br>%{theta:.1f}° (Param: %{customdata[0]:.2f})<extra></extra>'
            ))
            fig.add_trace(go.Scatterpolar(
                ...
                customdata=list(zip(p_vals, rs_weights_raw)),
                hovertemplate='<b>β_rs Weight</b>: %{r:.3f} (norm)<br>Raw: %{customdata[1]:.3f}<br>%{theta:.1f}° (Param: %{customdata[0]:.2f})<extra></extra>'
            ))
            fig.add_trace(go.Scatterpolar(
                ...
                customdata=list(zip(p_vals, c_weights_raw)),
                hovertemplate='<b>β_c Weight</b>: %{r:.3f} (norm)<br>Raw: %{customdata[1]:.3f}<br>%{theta:.1f}° (Param: %{customdata[0]:.2f})<extra></extra>'
            ))
            fig.add_trace(go.Scatterpolar(
                ...
                customdata=list(zip(p_vals, attn_weights_raw)),
                hovertemplate='<b>Attention Weight</b>: %{r:.3f} (norm)<br>Raw: %{customdata[1]:.3f}<br>%{theta:.1f}° (Param: %{customdata[0]:.2f})<extra></extra>'
            ))
            fig.add_trace(go.Scatterpolar(
                ...
                customdata=list(zip(p_vals, combined_weights_raw)),
                hovertemplate='<b>Hybrid Weight</b>: %{r:.3f} (norm)<br>Raw: %{customdata[1]:.3f}<br>%{theta:.1f}° (Param: %{customdata[0]:.2f})<extra></extra>'
            ))
            # Set tick labels to avoid overlap: show every 45 degrees or based on number of points
            n_ticks = min(8, len(angles))
            tick_indices = np.linspace(0, len(angles)-1, n_ticks, dtype=int)
            tick_vals = angles[tick_indices]
            tick_text = [f'{p_vals[i]:.1f}' for i in tick_indices]
            fig.update_layout(
                title=dict(
                    text=f'Physics-Aware Radar by {pname}',
                    x=0.5,
                ...
                annotations=[
                    dict(
                        text='Radial: Normalized weights (0-1, raw in hover). Angles: Sorted parameter values (min at 0°, max at 360°). Physics refinement incorporated in weights.',
                        x=0.5, y=-0.1, showarrow=False, font=dict(size=12)
                    )
                ],
                polar=dict(
...
# =============================================
# HELPER FOR DYNAMIC PLOTLY STYLING
# =============================================
def apply_global_styles(fig, font_family, font_size, font_color, bg_color):
    fig.update_layout(
        ...
        margin=dict(l=80, r=80, t=120, b=80, pad=10),
        legend=dict(
            orientation="h",
            yanchor="bottom",
            y=-0.15,
...
def main():
    ...
    with st.sidebar:
        ...
        font_family = st.selectbox("Font Family", ["Arial", "Helvetica", "Times New Roman", "Courier New"], index=0)
        font_size = st.slider("Font Size", 8, 24, 14, 1)
        font_color = st.color_picker("Font Color", "#000000")
        background_color = st.color_picker("Background Color", "#FFFFFF")
       
        # Update global matplotlib rcParams
        plt.rcParams.update({
        ...
        # Update weight_visualizer font_config
        st.session_state.weight_visualizer.font_config['family'] = font_family
        st.session_state.weight_visualizer.font_config['size_title'] = font_size + 4
        st.session_state.weight_visualizer.font_config['size_labels'] = font_size
        st.session_state.weight_visualizer.font_config['size_ticks'] = font_size - 2
