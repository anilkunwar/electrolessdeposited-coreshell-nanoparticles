...
9. ENHANCED VISUAL STYLING - Global font controls, expanded colormaps, clearer radar annotations
"""
...
# =============================================
# GLOBAL STYLING CONFIGURATION (MATPLOTLIB)
# =============================================
plt.rcParams.update({
    'font.size': 14,
    'axes.titlesize': 20,
...
COLORMAP_OPTIONS = {
    ...
    'Sequential (Scientific)': ['binary', 'gist_yarg', 'gist_gray', 'gray', 'bone', 'pink',
...
# For Plotly, we'll use a list of valid colorscales (subset)
PLOTLY_COLORSCALES = ['viridis', 'plasma', 'inferno', 'magma', 'cividis', 'turbo',
                      'RdBu', 'RdYlBu', 'Spectral', 'coolwarm', 'Portland', 'Electric',
                      'Blackbody', 'Bluered', 'Blues', 'Earth', 'Greens', 'Greys',
                      'Hot', 'Jet', 'Picnic', 'Rainbow', 'reds', 'YlGnBu']
...
# =============================================
# HYBRID WEIGHT VISUALIZER (ENHANCED STYLING)
# =============================================
class HybridWeightVisualizer:
    ...
    Supports global styling via font and color settings.
    """
    
    def __init__(self, font_family='Arial, sans-serif', font_size_title=24,
                 font_size_labels=18, font_size_ticks=14, font_color='#2C3E50',
    ...
    def _apply_global_layout(self, fig, title=None, showlegend=True):
        """Apply global styling settings to a Plotly figure."""
        fig.update_layout(
            font=dict(family=self.font_family, size=self.font_size_labels, color=self.font_color),
            plot_bgcolor=self.plot_bgcolor,
            paper_bgcolor=self.paper_bgcolor,
            title=dict(
                text=title,
                font=dict(family=self.font_family, size=self.font_size_title, color=self.font_color),
                x=0.5, y=0.95, xanchor='center', yanchor='top',
                pad=dict(b=20)
            ) if title else None,
            showlegend=showlegend,
            legend=dict(
                font=dict(family=self.font_family, size=self.font_size_ticks, color=self.font_color),
                orientation='h', yanchor='bottom', y=1.02, xanchor='center', x=0.5,
                bgcolor='rgba(255,255,255,0.8)', bordercolor='black', borderwidth=1,
                itemwidth=50, tracegroupgap=10
            ),
            hoverlabel=dict(
                font=dict(family=self.font_family, size=self.font_size_ticks, color='white'),
                bgcolor='rgba(44,62,80,0.9)', bordercolor='white'
            )
        )
        return fig
    ...
    def create_enhanced_sankey_diagram(self, sources_data, target_params, param_sigmas):
        ...
        title = f'<b>SANKEY DIAGRAM: HYBRID WEIGHT COMPONENT FLOW</b><br>' + \
                f'Target: L0={target_params.get("L0_nm", 20):.0f}nm, fc={target_params.get("fc", 0.18):.2f}'
        self._apply_global_layout(fig, title=title)
        fig.update_layout(width=1400, height=900, margin=dict(t=100, l=50, r=50, b=50))
        return fig
    ...
    def create_enhanced_chord_diagram(self, sources_data, target_params):
        ...
        fig.add_trace(go.Scatter(
            ...
            textfont=dict(size=16, color='white', family=self.font_family),
            hoverinfo='text',
        ...
        title = f'<b>ENHANCED CHORD DIAGRAM: HYBRID WEIGHT VISUALIZATION</b><br>' + \
                f'Target: L0={target_params.get("L0_nm", 20):.0f}nm, fc={target_params.get("fc", 0.18):.2f}'
        self._apply_global_layout(fig, title=title)
        fig.update_layout(
            width=1200, height=1000, hovermode='closest',
    ...
    # Parameter‑based radar charts with enhanced hover and annotations
    def create_parameter_radar_charts(self, sources_data, target_params, param_sigmas):
        ...
        Hover shows actual parameter value and raw weight.
        """
        ...
        for pkey, pname in zip(param_keys, param_names):
            ...
            # Customdata for hover: actual parameter value and raw weights
            customdata_l0 = np.stack([p_vals, l0_weights, fc_weights, rs_weights,
                                      c_weights, attn_weights, combined_weights], axis=-1)
            ...
            fig.add_trace(go.Scatterpolar(
                ...
                hovertemplate='<b>α (L0) Weight</b><br>' +
                              f'{pname}: %{{customdata[0]:.3f}}<br>' +
                              'Normalized: %{r:.3f}<br>' +
                              'Raw: %{customdata[1]:.4f}<br>' +
                              '<extra></extra>'
            ))
            fig.add_trace(go.Scatterpolar(
                ...
                customdata=customdata_l0,
                hovertemplate='<b>β_fc Weight</b><br>' +
                              f'{pname}: %{{customdata[0]:.3f}}<br>' +
                              'Normalized: %{r:.3f}<br>' +
                              'Raw: %{customdata[2]:.4f}<br>' +
                              '<extra></extra>'
            ))
            fig.add_trace(go.Scatterpolar(
                ...
                customdata=customdata_l0,
                hovertemplate='<b>β_rs Weight</b><br>' +
                              f'{pname}: %{{customdata[0]:.3f}}<br>' +
                              'Normalized: %{r:.3f}<br>' +
                              'Raw: %{customdata[3]:.4f}<br>' +
                              '<extra></extra>'
            ))
            fig.add_trace(go.Scatterpolar(
                ...
                customdata=customdata_l0,
                hovertemplate='<b>β_c Weight</b><br>' +
                              f'{pname}: %{{customdata[0]:.3f}}<br>' +
                              'Normalized: %{r:.3f}<br>' +
                              'Raw: %{customdata[4]:.4f}<br>' +
                              '<extra></extra>'
            ))
            fig.add_trace(go.Scatterpolar(
                ...
                customdata=customdata_l0,
                hovertemplate='<b>Attention</b><br>' +
                              f'{pname}: %{{customdata[0]:.3f}}<br>' +
                              'Normalized: %{r:.3f}<br>' +
                              'Raw: %{customdata[5]:.4f}<br>' +
                              '<extra></extra>'
            ))
            fig.add_trace(go.Scatterpolar(
                ...
                customdata=customdata_l0,
                hovertemplate='<b>Hybrid Weight</b><br>' +
                              f'{pname}: %{{customdata[0]:.3f}}<br>' +
                              'Normalized: %{r:.3f}<br>' +
                              'Raw: %{customdata[6]:.4f}<br>' +
                              '<extra></extra>'
            ))

            # Add annotation explaining normalization
            annotation_text = f"Radial values normalized to max combined weight = {max_w:.4f}<br>Hover for raw values."
            fig.add_annotation(
                text=annotation_text,
                xref="paper", yref="paper",
                x=0.5, y=-0.1,
                showarrow=False,
                font=dict(family=self.font_family, size=self.font_size_ticks, color='gray')
            )

            title = f'Radar by {pname}'
            self._apply_global_layout(fig, title=title)
            fig.update_layout(
                polar=dict(
                    radialaxis=dict(range=[0, 1.05],
                                   tickfont=dict(family=self.font_family, size=self.font_size_ticks),
    ...
    def create_weight_formula_breakdown(self, sources_data, target_params, param_sigmas):
        ...
        fig.add_trace(go.Bar(
            ...
            text=[f'{v:.3f}' for v in l0_values], textposition='outside',
            textfont=dict(family=self.font_family, size=10),
        ...
        fig.add_trace(go.Bar(
            ...
            text=[f'{v:.3f}' for v in fc_values], textposition='outside',
            textfont=dict(family=self.font_family, size=10)
        ), row=1, col=1)
        ...
        fig.add_trace(go.Bar(
            ...
            text=[f'{v:.3f}' for v in attention_values], textposition='outside',
            textfont=dict(family=self.font_family, size=10)
        ), row=1, col=1)
        ...
        title = f'<b>HYBRID WEIGHT FORMULA ANALYSIS</b><br>' + \
        ...
        for i in range(1,3):
            for j in range(1,3):
                fig.update_xaxes(title_font=dict(family=self.font_family, size=14, color=self.font_color), row=i, col=j)
                fig.update_yaxes(title_font=dict(family=self.font_family, size=14, color=self.font_color), row=i, col=j)
...
class HeatMapVisualizer:
    def __init__(self):
        ...
        self.plotly_colorscales = PLOTLY_COLORSCALES
...
def main():
    ...
    if 'weight_visualizer' not in st.session_state:
        st.session_state.weight_visualizer = None  # will be initialized with styling
    ...
    with st.sidebar:
        ...
        # Figure styling panel
        with st.expander("🎨 Figure Styling", expanded=False):
            font_family = st.selectbox("Font Family", 
                                      ['Arial, sans-serif', 'Helvetica, sans-serif', 
                                       'Times New Roman, serif', 'Courier New, monospace'],
                                      index=0)
            font_size_title = st.slider("Title Font Size", 16, 32, 24, 1)
            font_size_labels = st.slider("Label Font Size", 12, 24, 18, 1)
            font_size_ticks = st.slider("Tick Font Size", 10, 20, 14, 1)
            font_color = st.color_picker("Font Color", "#2C3E50")
            plot_bgcolor = st.color_picker("Plot Background", "#F0F0F5")
    ...
    if st.session_state.temporal_manager:
        ...
        with tabs[0]:
            ...
            # Flatten colormap options for dropdown
            all_cmaps = []
            for category, cmaps in COLORMAP_OPTIONS.items():
                all_cmaps.extend(cmaps)
            cmap = st.selectbox("Colormap", all_cmaps, index=all_cmaps.index('viridis') if 'viridis' in all_cmaps else 0)
        ...
        with tabs[1]:
            ...
            # Ensure weight visualizer is initialized
