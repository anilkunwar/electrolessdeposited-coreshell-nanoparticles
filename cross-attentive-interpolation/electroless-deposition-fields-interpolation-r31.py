...
- NEW in multi‑prediction comparison:
    ...
    * Parallel coordinates (overview of all parameters and metrics)
        ...
        for p, lbl in zip(predictions, labels):
            ...
            for col in columns:
                if col in p['target_params']:
                    ...
                    # weights might be dict, entropy is scalar
                    if col == 'entropy':
                        row[col] = p['weights'].get('entropy', 0)
                    else:
                        row[col] = np.mean(p['weights'].get(col, [0]))  # average if list
                else:
                    row[col] = 0
            data.append(row)
            ...
            if not st.session_state.saved_predictions:
                ...
                if selected_indices:
                    ...
                    if plot_type == "Thickness Evolution":
                        ...
                        # Choose three parameters for axes, plus color and size metrics
                        ...
                        # Let user select dimensions to include
                        ...
                        # Build dimension list for the plot
                        dims = []
