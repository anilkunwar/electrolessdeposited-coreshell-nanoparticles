...
✅ FIXED: StreamlitDuplicateElementId - removed duplicate download buttons
...
def render_design_panel(design: DesignConfig, key_prefix: str = "main") -> DesignConfig:
    ...
    with st.expander("🎨 Visualization Design Controls", expanded=False):
        ...
        with col_t1:
            ...
            design.title_font_weight = st.selectbox(
                "Title Weight", ["normal", "bold", "bolder"],
        ...
        with col_c1:
            ...
            design.colormap_reversed = st.checkbox(
                "Reverse Colormap", value=design.colormap_reversed,
        ...
        with col_l1:
            design.line_width = float(st.slider(
        ...
        with col_l2:
            ...
            design.marker_line_width = float(st.slider(
        ...
        design.annotation_enabled = st.checkbox(
            "Add Annotation", value=design.annotation_enabled,
            key=f"{key_prefix}_annot_enable"
        )
        if design.annotation_enabled:
            design.annotation_text = st.text_input(
                "Annotation Text", design.annotation_text,
                key=f"{key_prefix}_annot_text"
            )
            annot_x = int(st.slider(
                "X Position %", 0, 100, int(design.annotation_pos[0]*100), 5,
                key=f"{key_prefix}_annot_x"
            ))
            annot_y = int(st.slider(
                "Y Position %", 0, 100, int(design.annotation_pos[1]*100), 5,
    ...
    with tab1:
        ...
        if not summary_table.empty:
            ...
            with col_exp1:
                ...
                # ✅ FIXED: Removed duplicate download button - only using new width='stretch' parameter
                st.download_button(
                    "📥 Download CSV", 
                    csv, 
