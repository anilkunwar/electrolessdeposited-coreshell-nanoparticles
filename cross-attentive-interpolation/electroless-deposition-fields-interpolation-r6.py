...
FULL TEMPORAL SUPPORT + MEMORY-EFFICIENT ARCHITECTURE
...
class TemporalFieldManager:
    ...
    def get_fields(self, time_norm: float, use_interpolation: bool = True) -> Dict[str, np.ndarray]:
        ...
        if use_interpolation and self.key_frames:
            ...
            self._add_to_lru(t_key, interp_fields, interp_thickness)
        ...
        if res:
            self._add_to_lru(t_key, res['fields'], res['derived']['thickness_nm'])
            return res['fields']
    ...
    def _add_to_lru(self, time_norm: float, fields: Dict[str, np.ndarray], thickness_nm: float):
...
class CoreShellInterpolator:
    ...
    def interpolate_fields(self, sources: List[Dict], target_params: Dict,
        ...
        # Compute growth rate if thickness history available
        growth_rate = 0.0
        if time_norm is not None and len(thickness_curves) > 0:
            # Approximate growth rate from interpolated curve
            idx = int(time_norm * (len(common_t_norm) - 1))
            if idx > 0:
                dt = common_t_norm[idx] - common_t_norm[idx-1]
                if dt > 0:
                    growth_rate = (thickness_interp[idx] - thickness_interp[idx-1]) / dt
...
class HeatMapVisualizer:
    ...
    def create_field_heatmap(self, field_data, title, cmap_name='viridis',
                              ...
                              vmin=None, vmax=None, target_params=None, time_norm=None):
    ...
    def create_interactive_heatmap(self, field_data, title, cmap_name='viridis',
                                    ...
                                    target_params=None, time_norm=None):
    ...
    def create_thickness_plot(self, thickness_time, source_curves=None, weights=None,
                              ...
                              current_time=None, show_growth_rate=False):
        fig, ax = plt.subplots(figsize=figsize, dpi=300)
        t_norm = np.array(thickness_time['t_norm'])
        ...
        if show_growth_rate and len(t_norm) > 1:
            growth_rate = np.gradient(th_nm, t_norm)
            ax2 = ax.twinx()
            ax2.plot(t_norm, growth_rate, 'g--', linewidth=2, alpha=0.7, label='Growth rate')
            ax2.set_ylabel('Growth Rate (nm/t)', fontsize=12, color='green')
        ...
        if current_time is not None:
            ...
            ax.plot(current_time, current_th, 'ro', markersize=8, label=f'Current: t={current_time:.2f}, h={current_th:.2f} nm')
...
class ResultsManager:
    ...
    def export_to_json(self, export_data, filename=None):
        if filename is None:
            ...
            rs = p.get('rs', 0)
            cb = p.get('c_bulk', 0)
            t = export_data['result'].get('time_norm', 0)
            filename = f"temporal_interp_fc{fc:.3f}_rs{rs:.3f}_c{cb:.2f}_t{t:.3f}_{ts}.json"
    ...
    def export_to_csv(self, interpolation_result, filename=None):
        if filename is None:
            ...
            rs = p.get('rs', 0)
            cb = p.get('c_bulk', 0)
            t = interpolation_result.get('time_norm', 0)
            filename = f"fields_fc{fc:.3f}_rs{rs:.3f}_c{cb:.2f}_t{t:.3f}_{ts}.csv"
        ...
        X, Y = np.meshgrid(x, y)
        data = {'x_nm': X.flatten(), 'y_nm': Y.flatten(), 'time_norm': interpolation_result.get('time_norm', 0)}
...
def main():
    ...
    if st.session_state.temporal_manager:
        ...
        with col_time1:
            current_time = st.slider("Normalized Time (0=start, 1=end)", 
                                    0.0, 1.0, 
        ...
        current_thickness = mgr.get_thickness_at_time(current_time)
        ...
        with col_info3:
            st.metric("Time", f"{current_time:.3f}")
        ...
        with tabs[0]:
            ...
            fields = mgr.get_fields(current_time, use_interpolation=True)
            ...
            if st.checkbox("Show temporal evolution grid"):
                ...
                times_compare = np.linspace(0, 1, n_compare)
                ...
                fig_grid = st.session_state.visualizer.create_temporal_comparison_plot(
                    fields_list, times_compare, field_key=field_key,
        ...
        with tabs[1]:
            ...
            t_arr = np.array(thickness_time['t_norm'])
            
            stats_cols = st.columns(4)
            with stats_cols[0]:
                st.metric("Final Thickness", f"{th_arr[-1]:.3f} nm")
            with stats_cols[1]:
                st.metric("Initial Growth Rate", f"{(th_arr[1]-th_arr[0])/(t_arr[1]-t_arr[0]):.3f} nm/t")
            with stats_cols[2]:
                avg_rate = (th_arr[-1] - th_arr[0]) / (t_arr[-1] - t_arr[0])
                st.metric("Avg Growth Rate", f"{avg_rate:.3f} nm/t")
            with stats_cols[3]:
                st.metric("Time to 50% thickness", 
                         f"{t_arr[np.argmin(np.abs(th_arr - 0.5*th_arr[-1]))]:.3f}")
        ...
        with tabs[2]:
            ...
            if anim_method == "Real-time interpolation":
                ...
                if st.button("▶️ Play Animation", use_container_width=True):
                    ...
                    for t in times:
                        fields = mgr.get_fields(t, use_interpolation=True)
                        fig = st.session_state.visualizer.create_field_heatmap(
                            fields[field_key],
                            title=f"t = {t:.3f}",
                            cmap_name=cmap,
        ...
        with tabs[3]:
            ...
            res = st.session_state.interpolator.interpolate_fields(
                ...
                n_time_points=100, time_norm=current_time
            )
            ...
            if res:
                ...
                with col2:
                    st.metric("Growth rate", f"{res['derived'].get('growth_rate', 0):.3f} nm/t")
                with col3:
                ...
                if st.button("📊 Export Current State (JSON)", use_container_width=True):
                    ...
                    if res:
                        export_data = st.session_state.results_manager.prepare_export_data(
                            res, {'cmap': cmap, 'field': field_key, 'time': current_time}
                        )
                ...
                with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                    for t in mgr.key_times:
                        ...
                        if res_t:
                            ...
                            zip_file.writestr(f"frame_t{t:.4f}.json", json_str)
