...
- **NEW**: GPT‑2 integration for advanced NLP parsing and intelligent completeness inference
"""
...
class NLParser:
    ...
    # NEW: GPT-2 based parsing method
    def parse_with_gpt2(self, text: str, gpt2_tokenizer, gpt2_model) -> dict:
        """Use GPT-2 to extract parameters from natural language."""
        if not text:
            return self.defaults.copy()
        
        # Build a prompt that instructs GPT-2 to output JSON
...
"""
        try:
            ...
            # Extract JSON part (assume last JSON object in output)
            json_start = generated.rfind('{')
            json_end = generated.rfind('}') + 1
            if json_start == -1 or json_end == 0:
                raise ValueError("No JSON found in GPT-2 output")
...
class CompletionAnalyzer:
    ...
    def compute_completion(manager, target_params: Dict, 
                          tolerance: float = 0.3,  # MODIFIED: increased from 0.1 to 0.3
                          use_median: bool = True,  # MODIFIED: added use_median
        ...
        Now uses median and a softer threshold."""
    ...
    # NEW: GPT-2 based completeness inference
    @staticmethod
...
Output as JSON: {{ "complete": bool, "t_complete": float or null, "dr_min": float or null, "explanation": str }}
"""
        try:
            inputs = gpt2_tokenizer.encode(prompt, return_tensors='pt', truncation=True, max_length=512)
            if torch.cuda.is_available():
