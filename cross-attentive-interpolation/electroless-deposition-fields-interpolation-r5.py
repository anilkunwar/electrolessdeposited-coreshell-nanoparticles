...
# -*- coding: utf-8 -*-
"""
ELECTROLESS Ag — TRANSFORMER INTERPOLATOR (FULLY EXPANDED)
* Real Gated Attention (EDL vs no-EDL heads)
...
# =============================================
# CORE‑SHELL INTERPOLATOR with REAL GATED ATTENTION
# =============================================
class CoreShellInterpolator:
    ...
    def interpolate_fields(self, sources: List[Dict], target_params: Dict,
        ...
        """Main interpolation with EDL-aware dual-gated attention."""
        if not sources:
            return None
        ...
        # === Encode parameters ===
        target_feat = self.encode_parameters([target_params])
        edl_feat = self.encode_parameters(edl_params) if edl_params else torch.zeros((0, 12))
        no_edl_feat = self.encode_parameters(no_edl_params) if no_edl_params else torch.zeros((0, 12))
        
        # === Transformer embedding ===
        all_feat = torch.cat([target_feat, edl_feat, no_edl_feat], dim=0).unsqueeze(0)
        proj = self.pos_encoder(self.input_proj(all_feat))
        transformer_out = self.transformer(proj)
        
        target_rep = transformer_out[:, 0:1, :]
        edl_reps = transformer_out[:, 1:1+len(edl_params), :] if edl_params else None
        ...
        def compute_attention(target, sources, attention_module):
            ...
            attn_out, _ = attention_module(target, sources, sources)
        ...
        edl_kernel = self.compute_parameter_kernel(edl_params, target_params) if edl_params else np.array([])
        no_edl_kernel = self.compute_parameter_kernel(no_edl_params, target_params) if no_edl_params else np.array([])
        
        # === Dual gating with time-aware EDL decay ===
        time_tensor = torch.tensor([[time_norm]])
        
        if edl_reps is not None and edl_reps.shape[1] > 0:
            edl_gate_in = torch.cat([
                target_rep.expand(-1, edl_reps.shape[1], -1),
                edl_reps,
                time_tensor.expand(-1, edl_reps.shape[1], -1)
            ], dim=-1)
            edl_gate = self.edl_gate_net(edl_gate_in).squeeze(-1)
            edl_final = edl_gate * edl_scores + (1 - edl_gate) * torch.tensor(edl_kernel).unsqueeze(0)
        else:
            edl_final = torch.zeros((1, 0))
        
        if no_edl_reps is not None and no_edl_reps.shape[1] > 0:
            no_edl_gate_in = torch.cat([
                target_rep.expand(-1, no_edl_reps.shape[1], -1),
                no_edl_reps
            ], dim=-1)
        ...
        meta_edl_weight = self.meta_gate(meta_input).item()
        
        # === Combine and normalize weights ===
        edl_weights = torch.softmax(edl_final, dim=-1).squeeze().detach().cpu().numpy()
        no_edl_weights = torch.softmax(no_edl_final, dim=-1).squeeze().detach().cpu().numpy()
        ...
        return {
            ...
            'weights': {
                ...
                'edl': edl_weights.tolist(), 'no_edl': no_edl_weights.tolist(),
                'meta_edl_weight': meta_edl_weight,
                'kernel_edl': edl_kernel.tolist(), 'kernel_no_edl': no_edl_kernel.tolist()
            },
    ...
    if st.session_state.interpolation_result:
        ...
        with tabs[3]:  # Weights
            ...
            df_weights = pd.DataFrame({
                ...
                'Gate': res['weights'].get('gate', [0]*len(res['weights']['combined']))
            })
            st.dataframe(df_weights.style.format("{:.4f}"))
            st.metric("Meta EDL Weight", f"{res['weights'].get('meta_edl_weight', 0.0):.4f}")
