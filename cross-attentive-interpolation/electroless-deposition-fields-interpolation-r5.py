...
class CoreShellInterpolator:
    def __init__(self, d_model=64, nhead=8, num_layers=3,
        ...
        # Real multi‑head attention (used for both groups)
        self.attention = nn.MultiheadAttention(d_model, nhead, dropout=0.1, batch_first=True)

        # Dual gates: EDL gate (incorporates time_norm to model decay) and no‑EDL gate
        ...
        # Meta‑gate: blends EDL and no‑EDL contributions based on target use_edl and time_norm
    ...
    def _get_fields_at_time(self, source: Dict, time_norm: float, target_shape: Tuple[int, int]):
        ...
        If source has no snapshots or time_norm==1.0, returns final snapshot.
        """
        history = source.get('history', [])
        if not history:
            # No history – return zeros (should not happen)
        ...
        if len(history) == 1:
            ...
            # Find two snapshots surrounding t_target
            ...
            if t_target <= t_vals[0]:
                ...
                # Linear interpolation between two nearest snapshots
    ...
    def interpolate_fields(self, sources: List[Dict], target_params: Dict,
        ...
        # Use final state if time_norm not provided
        ...
        edl_reps = transformer_out[:, 1:1+len(edl_params), :] if edl_params else None  # (1, N_edl, d_model)
        no_edl_reps = transformer_out[:, 1+len(edl_params):, :] if no_edl_params else None

        # ---------- EDL group ----------
        if edl_reps is not None:
            # Real QKV attention for EDL group
            ...
            # Kernel weights for EDL group
            edl_kernel = self.compute_parameter_kernel(edl_params, target_params)       # (N_edl,)
            edl_kernel_t = torch.FloatTensor(edl_kernel).unsqueeze(0)                   # (1, N_edl)

            # EDL gate with time_norm (to model decay)
            time_t = torch.tensor([time_norm]).unsqueeze(0)                              # (1, 1)
            target_exp = target_rep.expand(edl_reps.shape[1], -1).unsqueeze(0)          # (1, N_edl, d_model)
            gate_input = torch.cat([target_exp, edl_reps, time_t.expand(-1, edl_reps.shape[1], 1)], dim=-1)  # (1, N_edl, 2*d_model+1)
            edl_gate = self.edl_gate_net(gate_input).squeeze(-1)                        # (1, N_edl)

            # Blend attention and kernel via gate
        ...
        # ---------- no‑EDL group ----------
        if no_edl_reps is not None:
            # Real QKV attention for no‑EDL group
            ...
            no_edl_kernel_t = torch.FloatTensor(no_edl_kernel).unsqueeze(0)

            target_exp = target_rep.expand(no_edl_reps.shape[1], -1).unsqueeze(0)
        ...
        # Convert to numpy weights, then combine with meta‑gate
        ...
        source_fields = []
        source_thickness = []   # for thickness curve interpolation

        for src in all_sources:
            fields = self._get_fields_at_time(src, time_norm, target_shape)
            source_fields.append(fields)

            # Thickness history (for curve, independent of time_norm)
        ...
        # ---------- Interpolate thickness evolution (full curve) ----------
        ...
        # ---------- Derived quantities at the current time ----------
        # Use thresholded material proxy
        ...
        result = {
            ...
            'derived': {
                ...
                'phase_stats': stats,
                'thickness_time': {                       # full evolution curve
            ...
            'weights': {
                ...
                'attention': (edl_scores.squeeze().detach().cpu().numpy().tolist() if edl_reps is not None else []) +
                             (no_edl_scores.squeeze().detach().cpu().numpy().tolist() if no_edl_reps is not None else []),
                'gate': (edl_gate.squeeze().detach().cpu().numpy().tolist() if edl_reps is not None else []) +
                        (no_edl_gate.squeeze().detach().cpu().numpy().tolist() if no_edl_reps is not None else []),
...
# =============================================
# HEATMAP VISUALIZER (unchanged, except minor additions)
# =============================================
...
# =============================================
# MAIN STREAMLIT APP (with global time slider)
# =============================================
def main():
...
# =============================================
# ENHANCED SOLUTION LOADER (unchanged from original)
# =============================================
