...
- FIXED: Tensor dimension mismatch in interpolate_fields (line 371-373)
"""
...
class CoreShellInterpolator:
    ...
    def interpolate_fields(self, sources: List[Dict], target_params: Dict,
        ...
        # ============================================================
        # FIX: Use (0, 12) for empty groups, NOT (1, self.d_model)
        # This was causing the RuntimeError: tensor dimension mismatch
        # ============================================================
        edl_features = self.encode_parameters(edl_params) if edl_params else torch.zeros((0, 12))
        no_edl_features = self.encode_parameters(no_edl_params) if no_edl_params else torch.zeros((0, 12))
        ...
        if all_features.shape[1] < 1:
            st.error("No valid sources available for interpolation")
            return None
