...
class NLParser:
    ...
    def parse_with_gpt2(self, text: str, gpt2_tokenizer, gpt2_model) -> dict:
...
def render_intelligent_designer_tab():
    ...
    with col_gpt1:
        use_gpt2_parse = st.checkbox("Use GPT-2 for parameter extraction (experimental)", 
                                      value=False, disabled=not gpt2_available,
                                      help="Requires transformers library and GPU/CPU memory")
    with col_gpt2:
    ...
    if run_design and user_input:
        with st.spinner("🔍 Parsing natural language input..."):
            ...
            if use_gpt2_parse and st.session_state.gpt2_tokenizer is not None:
                target_design = parser.parse_with_gpt2(user_input, st.session_state.gpt2_tokenizer, st.session_state.gpt2_model)
            else:
