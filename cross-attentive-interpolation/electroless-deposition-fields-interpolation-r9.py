...
# =============================================
# These RGBA values match the phase field data generator's max(phi,psi)+psi visualization
...
# Create matplotlib ListedColormap with exact colors (RGB only, alpha handled separately)
MATERIAL_COLORMAP_MATPLOTLIB = ListedColormap([
    MATERIAL_COLORS_EXACT['electrolyte'][:3],  # Red (electrolyte)
    MATERIAL_COLORS_EXACT['Ag'][:3],            # Orange (Ag)
    MATERIAL_COLORS_EXACT['Cu'][:3]             # Gray (Cu)
], name='phase_field_materials')

# BoundaryNorm places color boundaries at half-integers for crisp discrete mapping
# proxy=0 → [-0.5, 0.5) → electrolyte (red)
# proxy=1 → [0.5, 1.5)  → Ag (orange)
# proxy=2 → [1.5, 2.5]  → Cu (gray)
MATERIAL_BOUNDARY_NORM = BoundaryNorm([-0.5, 0.5, 1.5, 2.5], ncolors=3)

# Plotly discrete colorscale using exact RGB values (normalized to 0-1)
MATERIAL_COLORSCALE_PLOTLY = [
    [0.0, f"rgb({int(0.894*255)},{int(0.102*255)},{int(0.110*255)})"],  # Red electrolyte
    [0.333, f"rgb({int(0.894*255)},{int(0.102*255)},{int(0.110*255)})"],
    [0.334, f"rgb({int(1.000*255)},{int(0.498*255)},{int(0.000*255)})"],  # Orange Ag
    [0.666, f"rgb({int(1.000*255)},{int(0.498*255)},{int(0.000*255)})"],
    [0.667, f"rgb({int(0.600*255)},{int(0.600*255)},{int(0.600*255)})"],  # Gray Cu
...
class DepositionPhysics:
    ...
    def material_proxy(phi: np.ndarray, psi: np.ndarray, method: str = "max(phi, psi) + psi") -> np.ndarray:
        """
        Compute material proxy with EXACT phase-field threshold logic.
        
        Threshold rules (matching phase field generator):
        - Electrolyte (proxy=0): phi <= 0.5 AND psi <= 0.5
        - Ag shell (proxy=1):    phi > 0.5 AND psi <= 0.5
        - Cu core (proxy=2):     psi > 0.5 (regardless of phi)
        
        This matches the discrete colormap: red→electrolyte, orange→Ag, gray→Cu
        """
        if method == "max(phi, psi) + psi":
            # EXACT threshold-based discrete mapping
                ...
                if 'snapshots' in data and isinstance(data['snapshots'], list):
                    ...
                    for snap in data['snapshots']:
                        if isinstance(snap, tuple) and len(snap) == 4:
                            t, phi, c, psi = snap
                            snap_dict = {'t_nd': t, 'phi': self._ensure_2d(phi), 'c': self._ensure_2d(c), 'psi': self._ensure_2d(psi)}
...
# =============================================
# ENHANCED CORE-SHELL INTERPOLATOR WITH GATED L0 WEIGHTING
# =============================================
class CoreShellInterpolator:
    def __init__(self, d_model=64, nhead=8, num_layers=3, param_sigma=None, temperature=1.0):
        self.d_model = d_model
        self.nhead = nhead
    ...
    def compute_parameter_kernel(self, source_params: List[Dict], target_params: Dict):
        """Domain-specific kernel with weighted Euclidean distance and GATED L0 weighting."""
        ...
        target_norm = np.array([norm_val(target_params, 'fc'), norm_val(target_params, 'rs'),
                               norm_val(target_params, 'c_bulk'), norm_val(target_params, 'L0_nm')])
        
        base_weights = []
        for src in source_params:
            src_norm = np.array([norm_val(src, 'fc'), norm_val(src, 'rs'),
    ...
    def _get_fields_at_time(self, source: Dict, time_norm: float, target_shape: Tuple[int, int]):
        ...
        if len(history) == 1:
            ...
            if t_target <= t_vals[0]:
                snap = history[0]
                phi = self._ensure_2d(snap['phi']); c = self._ensure_2d(snap['c']); psi = self._ensure_2d(snap['psi'])
                ...
                phi1, phi2 = self._ensure_2d(snap1['phi']), self._ensure_2d(snap2['phi'])
                c1, c2 = self._ensure_2d(snap1['c']), self._ensure_2d(snap2['c'])
                psi1, psi2 = self._ensure_2d(snap1['psi']), self._ensure_2d(snap2['psi'])
        ...
        if phi.shape != target_shape:
            ...
            phi = zoom(phi, factors, order=1); c = zoom(c, factors, order=1); psi = zoom(psi, factors, order=1)
        return {'phi': phi, 'c': c, 'psi': psi}
    
    def interpolate_fields(self, sources: List[Dict], target_params: Dict,
                          target_shape: Tuple[int, int] = (256, 256),
                          n_time_points: int = 100, time_norm: Optional[float] = None):
        if not sources:
            return None
        ...
        result = {
            'fields': interp,
            'derived': {
                'material': material, 'potential': potential, 'thickness_nm': thickness_nm,
                'growth_rate': growth_rate, 'phase_stats': stats,
                'thickness_time': {'t_norm': common_t_norm.tolist(), 't_real_s': common_t_real.tolist(), 'th_nm': thickness_interp.tolist()}
            },
            'weights': {
                'combined': final_weights.tolist(), 'kernel': kernel_weights.tolist(),
                'attention': attn_scores.squeeze().detach().cpu().numpy().tolist(), 'entropy': float(entropy)
            },
            'target_params': target_params, 'shape': target_shape, 'num_sources': len(source_fields),
            'source_params': source_params, 'time_norm': t_req, 'time_real_s': t_real,
    ...
    def prepare_export_data(self, interpolation_result, visualization_params):
        ...
        export = {
            ...
            'result': {
                ...
                'time_norm': res.get('time_norm', 1.0), 'time_real_s': res.get('time_real_s', 0.0),
...
def main():
    ...
    # Material color legend for reference
    st.markdown("""
    ...
    with st.sidebar:
        ...
        n_key_frames = st.slider("Key frames for temporal interpolation", 5, 20, 10, 1, help="More frames = smoother animation but more memory")
        lru_cache_size = st.slider("Interactive cache size", 1, 5, 3, 1, help="Frames to keep in memory for slider responsiveness")
        
        target = {
            'fc': fc, 'rs': rs, 'c_bulk': c_bulk, 'L0_nm': L0_nm,
            'bc_type': bc_type, 'use_edl': use_edl, 'mode': mode,
            'growth_model': growth_model, 'alpha_nd': alpha_nd, 'tau0_s': tau0_target
        }
    ...
    if st.session_state.temporal_manager:
        ...
        with col_time1:
            current_time_norm = st.slider("Normalized Time (0=start, 1=end)", 0.0, 1.0,
                                         value=st.session_state.current_time, step=0.001, format="%.3f")
        ...
        with tabs[0]:
            ...
            field_choice = st.selectbox("Select field", ['c (concentration)', 'phi (shell)', 'psi (core)', 'material proxy'], key='field_choice')
            ...
            fig = st.session_state.visualizer.create_field_heatmap(
                ...
                target_params=target, time_real_s=current_time_real,
            ...
            if st.checkbox("Show interactive heatmap"):
                fig_inter = st.session_state.visualizer.create_interactive_heatmap(
                    field_data, title=f"Interactive {field_choice}", cmap_name=cmap, L0_nm=L0_nm,
                    target_params=target, time_real_s=current_time_real
                )
        ...
        with tabs[1]:
            ...
            fig_th = st.session_state.visualizer.create_thickness_plot(
                thickness_time, title=f"Shell Thickness Evolution (fc={fc:.3f}, rs={rs:.3f}, c_bulk={c_bulk:.2f})",
                current_time_norm=current_time_norm, current_time_real=current_time_real, show_growth_rate=show_growth
            )
            st.pyplot(fig_th)
            ...
            with stats_cols[0]: st.metric("Final Thickness", f"{th_arr[-1]:.3f} nm")
            with stats_cols[1]: st.metric("Initial Growth Rate", f"{(th_arr[1]-th_arr[0])/(t_arr[1]-t_arr[0]):.3f} nm/s")
            with stats_cols[2]:
            ...
            if anim_method == "Real-time interpolation":
                ...
                if st.button("▶️ Play Animation", use_container_width=True):
                    ...
                    for t_norm in times:
                        ...
                        fig = st.session_state.visualizer.create_field_heatmap(
                            field_data, title=f"t = {t_real:.3e} s", cmap_name=cmap, L0_nm=L0_nm,
                            target_params=target, time_real_s=t_real,
                            colorbar_label="Material" if field_key == 'material' else field_choice.split()[0]
                        )
                    ...
                    if mgr.animation_frame_paths:
                        ...
                        if st.button("▶️ Play Pre-rendered", use_container_width=True):
                            ...
                            for i, frame_path in enumerate(mgr.animation_frame_paths):
                                ...
                                field_data = DepositionPhysics.material_proxy(fields['phi'], fields['psi']) if field_key == 'material' else fields[field_key]
                                fig = st.session_state.visualizer.create_field_heatmap(
                                    field_data, title=f"t = {t_real:.3e} s [Pre-rendered]", cmap_name=cmap, L0_nm=L0_nm,
                                    target_params=target, time_real_s=t_real,
                                    colorbar_label="Material" if field_key == 'material' else field_choice.split()[0]
                                )
                                placeholder.pyplot(fig)
        ...
        with tabs[3]:
            ...
            if res:
                ...
                with col1: st.metric("Shell thickness (nm)", f"{res['derived']['thickness_nm']:.3f}")
                with col2: st.metric("Growth rate (nm/s)", f"{res['derived'].get('growth_rate', 0):.3f}")
                with col3: st.metric("Sources used", res['num_sources'])
                
                st.subheader("Phase Statistics")
                stats = res['derived']['phase_stats']
                cols = st.columns(3)
                with cols[0]:
                    st.metric("Electrolyte", f"{stats['Electrolyte'][0]:.4f} nd²", help=f"Real: {stats['Electrolyte'][1]*1e18:.2f} nm²")
                with cols[1]:
                    st.metric("Ag shell", f"{stats['Ag'][0]:.4f} nd²", help=f"Real: {stats['Ag'][1]*1e18:.2f} nm²")
                with cols[2]:
                    st.metric("Cu core", f"{stats['Cu'][0]:.4f} nd²", help=f"Real: {stats['Cu'][1]*1e18:.2f} nm²")
                ...
                with col_viz1:
                    ...
                    fig_mat = st.session_state.visualizer.create_field_heatmap(
                        res['derived']['material'], "Material Proxy", cmap_name='Set1', L0_nm=L0_nm, target_params=target,
                        colorbar_label="Material", vmin=0, vmax=2, time_real_s=current_time_real
                    )
                    st.pyplot(fig_mat)
                with col_viz2:
                    st.subheader("Potential Proxy")
                    fig_pot = st.session_state.visualizer.create_field_heatmap(
                        res['derived']['potential'], "Potential Proxy", cmap_name='RdBu_r', L0_nm=L0_nm, target_params=target,
        ...
        with tabs[4]:
            ...
            st.metric("Weight Entropy (Uncertainty)", f"{entropy:.4f}", help="Higher = more uncertain (sources contribute equally)")
            ...
            ax.set_xlabel('Source Index'); ax.set_ylabel('Weight'); ax.set_title('Interpolation Weights')
            ax.legend(); st.pyplot(fig_w)
        ...
        with tabs[5]:
            ...
            if st.button("📦 Export All Key Frames (ZIP)", use_container_width=True):
                ...
                st.download_button("⬇️ Download ZIP", zip_buffer.getvalue(), f"temporal_sequence_{target_hash}.zip", "application/zip")
    else:
        st.info("""
        👈 **Get Started:**
