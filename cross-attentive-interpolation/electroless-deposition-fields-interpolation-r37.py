...
def render_intelligent_designer_tab():
    ...
    if run_design and user_input:
        ...
        # SLIDER REPOSITIONED: moved above plots and given a clearer label (MODIFIED)
        slider_col1, slider_col2 = st.columns([3, 1])
        with slider_col1:
            selected_idx = st.slider(
                f"⏱️ Select time point (real time: {times_real[0]:.2e}s – {times_real[-1]:.2e}s)",
                0, len(times_norm) - 1, default_idx,
                format="Step %d"
            )
        with slider_col2:
            st.markdown(f"**t = {times_real[selected_idx]:.2e} s**")
