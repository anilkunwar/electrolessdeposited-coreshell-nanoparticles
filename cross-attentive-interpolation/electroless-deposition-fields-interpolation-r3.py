...
FULL TEMPORAL SUPPORT: interpolates fields (φ, c, ψ) at any desired time point,
in addition to the thickness evolution curve.
...
# =============================================
# CORE‑SHELL INTERPOLATOR with Gated Attention and Temporal Fields
# =============================================
class CoreShellInterpolator:
    ...
    def interpolate_fields(self, sources: List[Dict], target_params: Dict,
        ...
        If time_norm is None, uses the final state of each source (original behaviour).
        Otherwise, extracts fields at that normalised time from each source.
        """
        if not sources:
            return None
        ...
        for src in sources:
            ...
            # Process thickness history (always for the curve, independent of time_norm)
        ...
        # Encode parameters and compute attention weights (same for any time)
        ...
        # Interpolate thickness evolution (always done, independent of time_norm)
        ...
        result = {
            ...
            'derived': {
                ...
                'phase_stats': stats,
                'thickness_time': {          # thickness evolution curve
            ...
            'time_norm': time_norm if time_norm is not None else 1.0   # store the requested time
        }
        return result
        ...
        if st.button("🧠 Perform Initial Interpolation", type="primary", use_container_width=True):
            if not st.session_state.solutions:
                ...
                with st.spinner("Interpolating final state..."):
                    ...
                    # For initial display, use final state (time_norm=None)
                    ...
                    if res:
                        ...
                        # Also cache the final state
                        cache_key = (frozenset(target.items()), 1.0)  # time_norm=1.0
    ...
    if st.session_state.interpolation_result:
        ...
        with tabs[1]:
            ...
            if st.checkbox("Show source thickness curves (weighted transparency)"):
                # We would need to reconstruct source thickness from original sources.
                # For simplicity, we skip here.
                st.info("Source curves not displayed in this demo.")
