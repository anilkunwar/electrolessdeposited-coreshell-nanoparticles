...
--------------------------------------------------------
- Automatic scale bar detection + OCR
- Core diameter measurement (Hough circles / contour analysis)
- Composition extraction (Cu:Ag ratio) from EDS maps
- Fallback manual entry
- LLM query generation (GPT‑2 / Qwen)
"""
...
try:
    ...
    # Uncomment and set the correct path on Windows:
...
def detect_scale_bar(pil_img):
    ...
    # Step 2: OCR near detected lines
    scale_info = None
    best_confidence = 0.0
    ...
    # Step 3: Fallback: search entire image for "nm"
...
def detect_core_diameter(pil_img, scale_nm_per_px):
    ...
    # Fallback: edge + contour analysis
...
def extract_composition_from_image(pil_img):
    ...
    # Method 1: OCR for text patterns
    try:
    ...
    # Method 2: Color-based analysis (red=Cu, green=Ag)
    try:
...
def analyze_with_vision_llm(pil_img, task_description):
    """
    Optional: use GPT-4V or similar (requires API key).
    """
    try:
