...
8. Lexicographical L0 Prioritization & Dynamic Weight Shifting:
   - Primary ranking by L0 proximity (closest L0 dominates)
   - Secondary ranking by physics parameters (fc, rs, c_bulk) within L0 tier
   - For distant L0, physics similarity gains increased influence
   - Distant sources contribute with lower but non-zero weights (inclusivity)
"""
...
# =============================================
# ENHANCED CORE‑SHELL INTERPOLATOR WITH LEXICOGRAPHICAL L0 PRIORITIZATION AND DYNAMIC WEIGHT SHIFTING
# =============================================
class CoreShellInterpolator:
    def __init__(self, d_model=64, nhead=8, num_layers=3,
                 ...
                 tau_L0=5.0, tau_phys=2.0, L0_mixing_threshold=15.0, L0_mixing_scale=5.0):
        self.d_model = d_model
        self.nhead = nhead
        self.num_layers = num_layers
        if param_sigma is None:
            param_sigma = [0.15, 0.15, 0.10, 0.15]  # fc, rs, c_bulk, L0
        ...
        # NEW: Lexicographical L0 prioritization parameters
        self.tau_L0 = tau_L0                 # temperature for L0 softmax (smaller = sharper)
        self.tau_phys = tau_phys             # temperature for physics softmax
        self.L0_mixing_threshold = L0_mixing_threshold   # delta_L0 where mixing coefficient = 0.5
        self.L0_mixing_scale = L0_mixing_scale           # steepness of sigmoid
    ...
    def set_lexicographical_params(self, tau_L0, tau_phys, L0_mixing_threshold, L0_mixing_scale):
        self.tau_L0 = tau_L0
        self.tau_phys = tau_phys
        self.L0_mixing_threshold = L0_mixing_threshold
        self.L0_mixing_scale = L0_mixing_scale
    ...
    def compute_beta(self, source_params: List[Dict], target_params: Dict) -> np.ndarray:
        ...
        Uses weighted Gaussian with updated weights based on physical sensitivity:
        - c_bulk: 4.5 (highest influence)
        - fc: 3.5 (medium-high)
        - rs: 1.5 (lowest)
        """
        weights = {'fc': 3.5, 'rs': 1.5, 'c_bulk': 4.5}
        betas = []
        for src in source_params:
            sq_sum = 0.0
    ...
    def interpolate_fields(self, sources: List[Dict], target_params: Dict,
        ...
        # ========== NEW LEXICOGRAPHICAL L0 PRIORITIZATION AND DYNAMIC WEIGHT SHIFTING ==========
        target_L0 = target_params.get('L0_nm', 20.0)
        
        # Compute L0 deltas for each source
        delta_L0 = np.array([abs(src.get('L0_nm', 20.0) - target_L0) for src in source_params])
        
        # Compute L0 proximity weights using tiered alpha (or use delta directly with softmax)
        # We'll use alpha from the tiered method as a base L0 weight
        alpha = self.compute_alpha(source_params, target_L0)  # shape (n_sources,)
        
        # Compute physics similarity beta (weighted by physical sensitivity)
        beta = self.compute_beta(source_params, target_params)  # shape (n_sources,)
        
        # Compute shape similarity gamma and combine with beta
        beta_norm = beta / (np.sum(beta) + 1e-12)
        gamma = self.compute_gamma(source_fields, source_params, target_params, t_req, beta_norm)
        phys_sim = beta * (1.0 + self.lambda_shape * gamma)   # physics+shape similarity
        
        # Normalize to get probability distributions
        w_L0 = alpha / (np.sum(alpha) + 1e-12)
        w_phys = phys_sim / (np.sum(phys_sim) + 1e-12)
        
        # Compute mixing coefficient λ_i for each source based on its ΔL0
        # λ_i = sigmoid((threshold - delta_L0) / scale)  → near 1 for small delta, 0 for large delta
        lambda_i = 1.0 / (1.0 + np.exp((delta_L0 - self.L0_mixing_threshold) / self.L0_mixing_scale))
        
        # Combine weights: w = λ * w_L0 + (1-λ) * w_phys
        w_combined = lambda_i * w_L0 + (1.0 - lambda_i) * w_phys
        w_combined = w_combined / (np.sum(w_combined) + 1e-12)  # renormalize
        ...
        # Multiply attention scores by the physics-based combined weight
        final_scores = attn_scores * torch.FloatTensor(w_combined).unsqueeze(0)
        ...
        result = {
            ...
            'weights': {
                ...
                'gamma': gamma.tolist(),
                'w_L0': w_L0.tolist(),
                'w_phys': w_phys.tolist(),
                'lambda': lambda_i.tolist(),
                'w_combined_pre_attention': w_combined.tolist(),
...
def main():
    ...
    with st.sidebar:
        ...
        sigma_c = st.slider("Kernel σ (c_bulk)", 0.05, 0.3, 0.10, 0.01)
        ...
        # NEW: Lexicographical L0 prioritization parameters
        st.markdown("#### ⚖️ Lexicographical L0 Prioritization")
        tau_L0 = st.slider("τ_L0 (L0 softmax temperature)", 1.0, 20.0, 5.0, 0.5,
                          help="Smaller = sharper preference for closest L0.")
        tau_phys = st.slider("τ_phys (physics softmax temperature)", 0.5, 10.0, 2.0, 0.5,
                            help="Smaller = sharper preference for best physics match.")
        L0_mixing_threshold = st.slider("L0 mixing threshold (nm)", 5.0, 50.0, 15.0, 1.0,
                                        help="ΔL0 where mixing coefficient = 0.5.")
        L0_mixing_scale = st.slider("L0 mixing scale", 1.0, 20.0, 5.0, 0.5,
                                    help="Steepness of sigmoid; smaller = sharper transition.")
            ...
            if st.button("🧠 Initialize Temporal Interpolation", type="primary",
                ...
                if not st.session_state.solutions:
                    ...
                    with st.spinner("Setting up temporal interpolation..."):
                        ...
                        st.session_state.interpolator.set_lexicographical_params(
                            tau_L0, tau_phys, L0_mixing_threshold, L0_mixing_scale)
    ...
    if st.session_state.temporal_manager:
        ...
        with tabs[4]:
            ...
            if 'alpha' in weights:
                st.subheader("Lexicographical Weight Components")
                df_refine = pd.DataFrame({
                    'Source': range(len(weights['alpha'])),
                    'α (L0 tier)': weights['alpha'],
                    'β (physics)': weights['beta'],
                    'γ (shape)': weights['gamma'],
                    'w_L0 (norm)': weights['w_L0'],
                    'w_phys (norm)': weights['w_phys'],
                    'λ (mixing)': weights['lambda'],
                    'w_combined': weights['w_combined_pre_attention']
                })
                st.dataframe(df_refine.style.format("{:.4f}"))
            
            df_weights = pd.DataFrame({
                'Source': range(len(weights['combined'])),
                'Final Combined': weights['combined'],
