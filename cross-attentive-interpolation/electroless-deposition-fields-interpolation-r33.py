...
- Advanced weight visualizations: Sankey, chord, radar, breakdown
...
- CORRECTED SHELL THICKNESS: Now uses exact visual proxy (max(phi,psi)+psi) so thickness matches plot
- NEW in multi‑prediction comparison:
    * Matplotlib thickness evolution (publication quality)
    * 3D parameter scatter (5‑dimensional encoding)
    * Parallel coordinates (overview of all parameters and metrics) – now fixed for categorical labels
    * Pair plot (scatter matrix using Plotly)
- **INTELLIGENT DESIGNER**: Natural language input, relevance scoring (SciBERT), automatic completeness detection, minimal thickness extraction.
"""
...
class DepositionPhysics:
    ...
    def shell_thickness(phi: np.ndarray, psi: np.ndarray, core_radius_frac: float,
        ...
        """CORRECTED: Use the exact visual proxy so thickness matches the plot."""
        ...
        # Shell = exactly where the plot shows orange (proxy == 1.0)
        shell_mask = (proxy == 1.0)                                 # ← key change
...
class TemporalFieldManager:
    ...
    def _compute_thickness_curve(self):
        res = self.interpolator.interpolate_fields(
            ...
            recompute_thickness=True   # ← Use corrected definition
        )
        if res:
...
class CoreShellInterpolator:
    ...
    def interpolate_fields(self, sources: List[Dict], target_params: Dict,
                          ...
                          require_categorical_match: bool = False,
                          recompute_thickness: bool = True):   # ← NEW PARAM
        if not sources:
            return None
        ...
        for src in active_sources:
            ...
            if recompute_thickness:
                # Recompute thickness curve from raw fields (sample n_time_points)
                ...
                # Existing: Use pre‑computed thickness_history if available
...
class MultiPredictionVisualizer:
    """Generates comparison plots for multiple saved predictions with enhanced features."""
    ...
    def thickness_evolution_matplotlib(predictions, labels):
        """Matplotlib version of thickness evolution (publication quality)."""
    ...
    def thickness_evolution_3d(predictions, labels, param_y='fc', param_color='growth_rate', cmap='turbo'):
        """
        3D Plot of thickness evolution with a fourth parameter as colorscale.
        X: time (s), Y: selected parameter (e.g., fc), Z: thickness (nm), color: selected metric.
        """
        fig = go.Figure()
    ...
    def parameter_map(predictions, param_x, param_y, metric='thickness_nm', plot_type='scatter', cmap='viridis'):
        """
        2D contour/scatter of metric vs two parameters.
        If plot_type='scatter', uses px.scatter (size and color by metric).
        If plot_type='contour' and enough points, uses density contour or interpolation.
        """
        df = pd.DataFrame([
            {
    ...
    def weight_sunburst(predictions, labels, cmap='viridis'):
        ...
        Uses branchvalues='total' and colors sectors based on weight magnitude.
        """
        ids = ['all']
    ...
    # -------------- NEW MULTI‑DIMENSIONAL VISUALIZATIONS --------------
    @staticmethod
    def parameter_scatter_3d(predictions, labels, x_param, y_param, z_param,
        ...
        """
        3D scatter plot where each point is a prediction.
        Up to 5 dimensions: x, y, z from parameters; color and size from metrics.
        """
        df = pd.DataFrame([
            {
                'label': lbl,
    ...
    def parallel_coordinates(predictions, labels, dimensions):
        """
        Parallel coordinates plot for selected parameters and metrics.
        dimensions: list of dicts with 'label', 'values' (extracted from predictions).
        FIXED: handles categorical prediction labels by using a numeric color dimension.
        """
        df = pd.DataFrame()
        for dim in dimensions:
            df[dim['label']] = dim['values']
        # Use numeric IDs for coloring (categorical → numerical)
        df['pred_id'] = pd.factorize(labels)[0]
        df['prediction'] = labels  # keep for hover

        fig = px.parallel_coordinates(
            df,
            color='pred_id',  # use numeric IDs
        ...
        # Hide colorbar as it's categorical, not continuous
    ...
    def pairplot(predictions, labels, columns):
        """
        Scatter matrix (pair plot) using Plotly.
        columns: list of column names (parameters/metrics) to include.
        """
        data = []
        for p, lbl in zip(predictions, labels):
            row = {'label': lbl}
...
# =============================================
# NEW MODULES FOR INTELLIGENT DESIGNER
# =============================================
...
class NLParser:
    ...
    def __init__(self):
        self.defaults = {
            'fc': 0.18, 'rs': 0.2, 'c_bulk': 0.5, 'L0_nm': 60.0,
            'time': None,  # None means full evolution
            'bc_type': 'Neu', 'use_edl': True, 'mode': '2D (planar)',
            'alpha_nd': 2.0, 'tau0_s': 1e-4
        }
        self.patterns = {
            'L0_nm': r'L0=(\d+(?:\.\d+)?)\s*nm',
            'fc': r'fc=(\d+(?:\.\d+)?)',
            'rs': r'rs=(\d+(?:\.\d+)?)',
            'c_bulk': r'c_bulk=(\d+(?:\.\d+)?)',
            'time': r'time=(\d+(?:\.\d+)?(?:e[+-]?\d+)?)\s*s',
            'bc_type': r'bc_type=(\w+)',
            'use_edl': r'use_edl=(True|False)',
            'mode': r'mode=([23]D\s*\([^)]+\))',
            'alpha_nd': r'alpha_nd=(\d+(?:\.\d+)?)',
        }
    ...
    def parse(self, text: str) -> dict:
        ...
        for p in ['fc', 'rs', 'c_bulk', 'L0_nm']:
            ...
            if not (low <= params[p] <= high):
                st.warning(f"{p} out of range [{low}, {high}]; clipped.")
                params[p] = np.clip(params[p], low, high)
        return params


class RelevanceScorer:
    """Compute semantic relevance of user query to source data."""
    def __init__(self, use_scibert=True):
        ...
        if use_scibert:
            try:
                ...
                st.warning("SciBERT not available, using fallback.")
                self.use_scibert = False
    
    def encode_source(self, src_params):
        return f"Source: L0={src_params.get('L0_nm',20):.1f}nm, fc={src_params.get('fc',0.18):.3f}, rs={src_params.get('rs',0.2):.3f}, c_bulk={src_params.get('c_bulk',0.5):.2f}, mode={src_params.get('mode','2D')}, bc={src_params.get('bc_type','Neu')}, EDL={src_params.get('use_edl',True)}"
    
    def score(self, query: str, sources: List[Dict], weights: np.ndarray) -> float:
        if self.use_scibert and sources:
            src_texts = [self.encode_source(s['params']) for s in sources]
            query_emb = self.model.encode(query)
            src_embs = self.model.encode(src_texts)
            sims = np.dot(src_embs, query_emb) / (np.linalg.norm(src_embs, axis=1) * np.linalg.norm(query_emb) + 1e-12)
            return float(np.average(sims, weights=weights))
        else:
            # Fallback: use max weight as proxy for relevance
...
class CompletionAnalyzer:
    ...
    @staticmethod
    def compute_completion(manager: TemporalFieldManager, target_params: Dict, 
                           tolerance: float = 0.1) -> Tuple[float, float, bool]:
        """
        Scans key frames to find the earliest real time where shell is complete.
        Returns: (t_complete_s, dr_min_nm, is_complete_at_end)
        """
        key_times_norm = list(manager.key_frames.keys())
        if not key_times_norm:
            return 0.0, 0.0, False
        
        core_radius_nm = target_params['fc'] * target_params['L0_nm'] / 2
        struct = generate_binary_structure(2, 1)
        
        t_complete = None
        dr_min = None
        
        for t_norm in sorted(key_times_norm):
            fields = manager.key_frames[t_norm]
            proxy = DepositionPhysics.material_proxy(fields['phi'], fields['psi'])
            # Binarize shell (Ag) and core (Cu)
            shell = (proxy == 1)
            core = (proxy == 2)
            # Check continuity: after closing small gaps, shell should fully enclose core
            closed_shell = binary_closing(shell, structure=struct)
            # In 2D, check that every core pixel's 4-neighbors are not electrolyte
            # Simple proxy: ensure no electrolyte (0) is inside the region that should be shell
            # We'll use radial profile continuity instead:
            r, prof = DepositionPhysics.compute_radial_profile(proxy, target_params['L0_nm'])
            # Find core radius index
            core_idx = np.argmin(np.abs(r - core_radius_nm))
            # Check profile from core outward: should be >= 1 continuously
            if np.all(prof[core_idx:] >= 1.0 - tolerance):
                # Find where profile first drops below 1? Actually we want minimal thickness:
                # dr_min is radius where profile first reaches 1 (outer shell edge) minus core radius
                outer_idx = np.argmax(prof[core_idx:] < 1.0 - tolerance)
                if outer_idx == 0:  # no drop, shell extends to end of domain
                    outer_idx = len(prof[core_idx:]) - 1
                dr_est = r[core_idx + outer_idx] - core_radius_nm
                t_complete = manager.get_time_real(t_norm)
                dr_min = dr_est
                break
        
        if t_complete is None:
            t_complete = manager.get_time_real(max(key_times_norm))
            dr_min = manager.get_thickness_at_time(max(key_times_norm))
            return t_complete, dr_min, False
        else:
...
# =============================================
def main():
    st.set_page_config(page_title="Core‑Shell Deposition: Hybrid Weight Interpolation + Multi‑Prediction + Intelligent Designer",
    ...
    st.markdown('<h1 class="main-header">🧪 Core‑Shell Deposition: Hybrid Weight Interpolation + Multi‑Prediction + Intelligent Designer</h1>',
               unsafe_allow_html=True)
    ...
    if st.session_state.temporal_manager:
        ...
        tabs = st.tabs(["📊 Field Visualization", "📈 Thickness Evolution",
                       ...
                       "🤖 Intelligent Designer"])   # NEW TAB
        
        with tabs[0]:   # Field Visualization (unchanged)
            ...
            fig = st.session_state.visualizer.create_field_heatmap(
                ...
                time_real_s=mgr.get_time_real(current_time_norm),
        ...
        with tabs[1]:   # Thickness Evolution (unchanged)
        ...
        with tabs[2]:   # Animation (unchanged)
        ...
        with tabs[3]:   # Derived Quantities (unchanged)
        ...
        with tabs[4]:   # Hybrid Weight Analysis (unchanged)
                    ...
                    if res:
                        export_data = st.session_state.results_manager.prepare_export_data(
                            res, {'cmap': cmap, 'field': field_key,
                                 'time_norm': st.session_state.current_time,
                                 'time_real_s': mgr.get_time_real(st.session_state.current_time)}
                        )
            ...
            if not st.session_state.solutions:
                ...
                if sources_data:
                    ...
                    for idx, sol in enumerate(st.session_state.solutions):
                        ...
                        deltas = [
                            abs(DepositionParameters.normalize(target_local.get('fc', 0.18), 'fc') -
                               DepositionParameters.normalize(sol_params.get('fc', 0.18), 'fc')),
                            abs(DepositionParameters.normalize(target_local.get('rs', 0.2), 'rs') -
                               DepositionParameters.normalize(sol_params.get('rs', 0.2), 'rs')),
                            abs(DepositionParameters.normalize(target_local.get('c_bulk', 0.5), 'c_bulk') -
                               DepositionParameters.normalize(sol_params.get('c_bulk', 0.5), 'c_bulk')),
                            abs(DepositionParameters.normalize(target_local.get('L0_nm', 20.0), 'L0_nm') -
        ...
        with tabs[7]:   # Multi-Prediction Comparison (unchanged)
            st.markdown('<h2 class="section-header">📊 Multi-Prediction Comparison</h2>',
                       unsafe_allow_html=True)
            
            st.info("🔍 **Tip:** Hover over points for details, zoom and pan to explore. For best results, select 3+ predictions.")
        ...
        with tabs[8]:   # Intelligent Designer
            st.markdown('<h2 class="section-header">🤖 Intelligent Designer</h2>',
                       unsafe_allow_html=True)
            
            st.markdown("""
            <div class="info-box">
            <strong>Design Goal:</strong> Enter a natural language description of your desired core‑shell nanoparticle.
            The system will extract parameters, estimate feasibility, and predict shell formation.
            <br><br>
            <em>Examples:</em>
            <ul>
            <li>"Design a core-shell with L0=50 nm, fc=0.2, c_bulk=0.3, time=1e-3 s for rs=0.2"</li>
            ...
            # Input area
            user_input = st.text_area("Enter your design request:", height=100,
                                      placeholder="e.g., Design a core-shell with L0=50 nm, fc=0.2, c_bulk=0.3, time=1e-3 s")
            
            col_design1, col_design2, col_design3 = st.columns([1,1,2])
            with col_design1:
            ...
            if run_design and user_input:
                ...
                # Override rs to 0.2 (design constraint)
                target_design['rs'] = 0.2
                
                # Display parsed parameters
                st.markdown("#### 📋 Parsed Parameters")
                col_parsed = st.columns(5)
                with col_parsed[0]:
                    st.metric("L0 (nm)", f"{target_design['L0_nm']:.1f}")
                with col_parsed[1]:
                    st.metric("fc", f"{target_design['fc']:.3f}")
                with col_parsed[2]:
                    st.metric("rs", f"{target_design['rs']:.3f}")
                with col_parsed[3]:
                    st.metric("c_bulk", f"{target_design['c_bulk']:.2f}")
                with col_parsed[4]:
                    t_str = f"{target_design['time']:.2e} s" if target_design['time'] else "Full evolution"
                    st.metric("Time", t_str)
                
                # Check if solutions are loaded
                if not st.session_state.solutions:
                    st.error("No solutions loaded. Please load solutions in the sidebar first.")
                else:
                    with st.spinner("Running intelligent design..."):
                        # Initialize a temporary manager for this design
                        # We'll reuse the global interpolator but create a new manager
                        # (this may be slow; we could cache if parameters repeat)
                        design_manager = TemporalFieldManager(
                            st.session_state.interpolator,
                            st.session_state.solutions,
                            target_design,
                            n_key_frames=5,  # smaller for speed
                            lru_size=2,
                            require_categorical_match=require_categorical_match
                        )
                        
                        # Relevance scoring
                        scorer = RelevanceScorer(use_scibert=use_scibert)
                        weights = design_manager.weights['combined'] if design_manager.weights else np.array([1.0])
                        relevance = scorer.score(user_input, st.session_state.solutions, weights)
                        
                        # Completion analysis
                        analyzer = CompletionAnalyzer()
                        t_complete, dr_min, complete_now = analyzer.compute_completion(design_manager, target_design)
                        
                        # Check if user-requested time is within evolution
                        if target_design['time'] is not None:
                            # Find normalized time corresponding to requested real time
                            t_max_real = design_manager.get_time_real(1.0)
                            t_norm_requested = target_design['time'] / t_max_real
                            if t_norm_requested > 1.0:
                                st.warning(f"Requested time {target_design['time']:.2e} s exceeds maximum evolution time {t_max_real:.2e} s. Using final state.")
                                t_norm_requested = 1.0
                            fields_requested = design_manager.get_fields(t_norm_requested)
                            proxy_requested = DepositionPhysics.material_proxy(fields_requested['phi'], fields_requested['psi'])
                            thickness_requested = design_manager.get_thickness_at_time(t_norm_requested)
                            complete_at_requested = (t_norm_requested >= t_complete / t_max_real) if t_complete else False
                        else:
                            t_norm_requested = 1.0
                            fields_requested = design_manager.get_fields(1.0)
                            proxy_requested = DepositionPhysics.material_proxy(fields_requested['phi'], fields_requested['psi'])
                            thickness_requested = design_manager.get_thickness_at_time(1.0)
                            complete_at_requested = complete_now
                        
                        # Display results
                        st.markdown("#### 🎯 Design Outputs")
                        
                        col_res1, col_res2, col_res3 = st.columns(3)
                        with col_res1:
                            st.metric("Relevance Score", f"{relevance:.3f}",
                                     help=">0.8: high confidence; 0.5-0.8: moderate; <0.5: low")
                            ...
                            if relevance > 0.8:
                                ...
                                st.warning("Low confidence – consider adjusting parameters")
                        
                        with col_res2:
                            st.metric("Minimal Complete Thickness", f"{dr_min:.2f} nm" if dr_min else "N/A")
                            if dr_min:
                                st.caption(f"Achieved at t = {t_complete:.2e} s")
                        
                        with col_res3:
                            if complete_at_requested:
                                st.success("✅ Complete shell at requested time")
                            else:
                                if t_complete:
                                    st.warning(f"❌ Not complete at requested time; completes at {t_complete:.2e} s")
                                else:
                                    st.error("❌ Shell never completes with these parameters")
                        
                        # Material proxy slider
                        st.markdown("#### 📊 Material Proxy at Requested Time")
                        st.caption("Red = electrolyte, Orange = Ag, Gray = Cu")
                        
                        # Use key frames for slider
                        ...
                        if target_design['time']:
                            idx_requested = np.argmin(np.abs(np.array(times_real) - target_design['time']))
                        else:
                            idx_requested = len(times_norm)-1
                        
                        selected_idx = st.slider("Time index", 0, len(times_norm)-1, idx_requested,
                                                format="%d")
                        t_sel_norm = times_norm[selected_idx]
                        t_sel_real = times_real[selected_idx]
                        fields_sel = design_manager.key_frames[t_sel_norm]
                        proxy_sel = DepositionPhysics.material_proxy(fields_sel['phi'], fields_sel['psi'])
                        
                        fig_proxy = st.session_state.visualizer.create_interactive_heatmap(
                            proxy_sel,
                            title=f"Material Proxy at t = {t_sel_real:.2e} s",
                            cmap_name='Set1',
                        ...
                        suggestions = []
                        if relevance < 0.5:
                            suggestions.append("• Low relevance: try parameters closer to existing sources (L0 around 40-60 nm, fc 0.15-0.25).")
                        if not complete_at_requested and t_complete:
                            if target_design['time']:
                                suggestions.append(f"• Increase time to at least {t_complete:.2e} s for completion.")
                            else:
                                suggestions.append("• Current parameters may not yield a complete shell. Try lower c_bulk (0.2-0.4) to promote complete coverage.")
                        if dr_min and dr_min > 10:
                            suggestions.append("• Minimal thickness >10 nm: consider reducing c_bulk or increasing fc to get thinner shells.")
                        if len(suggestions) == 0:
                            suggestions.append("• Your design looks promising! No major issues detected.")
                        
                        for s in suggestions:
                            st.markdown(s)
                        
                        # Option to save this design
                        if st.button("💾 Save This Design for Comparison"):
                            ...
                            res_design = st.session_state.interpolator.interpolate_fields(
                                st.session_state.solutions, target_design, target_shape=(256,256),
                                n_time_points=100, time_norm=t_norm_requested,
                                require_categorical_match=require_categorical_match,
                                recompute_thickness=True
                            )
                            if res_design:
                                st.session_state.saved_predictions.append(res_design)
                                st.success("Design saved. You can compare it in the Multi-Prediction tab.")
                            else:
        ...
        **Hybrid Weight Features:**
        ...
        • Multi-prediction comparison tools (now with 3D thickness, enhanced parameter maps, sunburst coloring, **plus Matplotlib thickness, 3D parameter scatter, parallel coordinates, and pair plot**)
        • **NEW Intelligent Designer**: natural language input, relevance scoring, automatic completeness detection, and minimal thickness extraction.
        """)
