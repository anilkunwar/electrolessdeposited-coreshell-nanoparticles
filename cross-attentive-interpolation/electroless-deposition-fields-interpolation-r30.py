...
- ENHANCED: Fixed thickness statistics with adjustable thresholds
- ENHANCED: Expanded colormap library (50+ options including jet, turbo, inferno, rainbow)
"""
...
from scipy.interpolate import interp1d, CubicSpline, RegularGridInterpolator, griddata
...
COLORMAP_OPTIONS = {
    'Sequential': [
        'viridis', 'plasma', 'inferno', 'magma', 'cividis', 'turbo', 'hot', 
        'afmhot', 'gist_heat', 'gray', 'bone', 'pink', 'spring', 'summer', 
        'autumn', 'winter', 'cool', 'Wistia', 'YlOrBr', 'YlOrRd', 'OrRd', 
        'PuRd', 'RdPu', 'BuPu', 'GnBu', 'PuBu', 'YlGnBu', 'PuBuGn', 'BuGn', 
        'YlGn', 'Greens', 'Blues', 'Purples', 'Reds', 'Oranges'
    ],
    'Diverging': [
        'RdBu', 'RdYlBu', 'Spectral', 'coolwarm', 'bwr', 'seismic', 'BrBG', 
        'PiYG', 'PRGn', 'PuOr', 'RdGy', 'RdYlGn', 'twilight', 'twilight_shifted'
    ],
    'Qualitative': [
        'tab10', 'tab20', 'tab20b', 'tab20c', 'Set1', 'Set2', 'Set3', 
        'Pastel1', 'Pastel2', 'Paired', 'Accent', 'Dark2', 'Paired'
    ],
    'Perceptually Uniform': [
        'viridis', 'plasma', 'inferno', 'magma', 'cividis', 'rocket', 
        'mako', 'flare', 'crest', 'icefire'
    ],
    'Publication Standard': [
        'viridis', 'plasma', 'inferno', 'magma', 'cividis', 'RdBu', 
        'coolwarm', 'seismic', 'gray', 'binary'
    ],
    'Cyclic': [
        'twilight', 'twilight_shifted', 'hsv', 'rainbow', 'turbo'
    ],
    'Miscellaneous': [
        'jet', 'turbo', 'rainbow', 'nipy_spectral', 'gist_rainbow', 
        'gist_earth', 'gist_stern', 'gist_ncar', 'ocean', 'terrain', 
        'flag', 'prism', 'brg', 'seismic', 'seismic_r'
    ]
}

# Flatten all colormaps for easy selection
ALL_COLORMAPS = []
for category, maps in COLORMAP_OPTIONS.items():
    ALL_COLORMAPS.extend(maps)
ALL_COLORMAPS = list(set(ALL_COLORMAPS))  # Remove duplicates
...
class DepositionPhysics:
    ...
    def shell_thickness(phi: np.ndarray, psi: np.ndarray, core_radius_frac: float,
                       threshold: float = 0.5, dx: float = 1.0, 
                       use_fallback: bool = True) -> float:
        """
        Compute shell thickness with adjustable threshold and fallback methods.
        
        Parameters:
        -----------
        phi : np.ndarray
            Phase field for Ag shell
        psi : np.ndarray
            Phase field for Cu core
        core_radius_frac : float
            Core radius as fraction of domain
        threshold : float
            Threshold for phi to detect shell (default 0.5, can lower to 0.3 for early growth)
        dx : float
            Grid spacing
        use_fallback : bool
            If True, use area-based fallback when mask detection fails
        
        Returns:
        --------
        float : Shell thickness in normalized units
        """
        ny, nx = phi.shape
        ...
        # Primary method: mask-based detection
        ...
        if np.any(mask):
            ...
            # Fallback method: area-based estimation
            if use_fallback:
                ag_mask = (phi > 0.3) & (psi <= 0.5)  # Softer threshold for area
                ag_area = np.sum(ag_mask) * dx * dx
                if ag_area > 0:
                    # Estimate thickness from area (assuming circular shell)
                    effective_radius = np.sqrt(ag_area / pi)
                    thickness = effective_radius - core_radius_frac
                    return max(0.0, thickness)
            return 0.0
    
    @staticmethod
    def shell_thickness_radial(phi: np.ndarray, psi: np.ndarray, core_radius_frac: float,
                               n_bins: int = 100, threshold: float = 0.3) -> float:
        """
        Alternative thickness measurement using radial profile integration.
        More robust for early growth stages.
        """
        ny, nx = phi.shape
        x = np.linspace(0, 1, nx)
        y = np.linspace(0, 1, ny)
        ...
        r_edges = np.linspace(0, np.sqrt(2)/2, n_bins + 1)
        ...
        for i in range(n_bins):
            ...
            if np.any(mask):
                ...
                psi_mean = np.mean(psi[mask])
                profile.append((phi_mean, psi_mean))
            else:
                profile.append((0.0, 0.0))
        
        profile = np.array(profile)
        
        # Find where phi exceeds threshold and psi is low (shell region)
        shell_mask = (profile[:, 0] > threshold) & (profile[:, 1] <= 0.5)
        if np.any(shell_mask):
            shell_radii = r_centers[shell_mask]
            max_shell_radius = np.max(shell_radii)
            thickness = max_shell_radius - core_radius_frac
            return max(0.0, thickness)
...
class CoreShellInterpolator:
    ...
    def interpolate_fields(self, sources: List[Dict], target_params: Dict,
        ...
        # FIX: Ensure final_weights is 1D array
        ...
        # ENHANCED: Use lower threshold (0.3) and fallback for better thickness detection
        thickness_nd = DepositionPhysics.shell_thickness(
            interp['phi'], interp['psi'], fc, 
            threshold=0.3, dx=dx, use_fallback=True
        )
...
# =============================================
# MULTI-PREDICTION COMPARISON VISUALIZER (ENHANCED)
# =============================================
class MultiPredictionVisualizer:
    """Generates comparison plots for multiple saved predictions with enhanced 3D visualizations."""
    ...
    @staticmethod
    def enhanced_thickness_3d(predictions, labels, param_y='fc', param_color='growth_rate', cmap='turbo'):
        """
        ENHANCED: 3D Plotly thickness evolution with fourth parameter colorscale.
        X-axis: time, Y-axis: selected parameter, Z-axis: thickness, Color: fourth parameter
        """
        data = []
        for pred, label in zip(predictions, labels):
            thick = pred['derived']['thickness_time']
            t = thick['t_real_s'] if 't_real_s' in thick else thick['t_norm']
            th = thick['th_nm']
            y_val = pred['target_params'].get(param_y, 0)
            
            # Get color value from derived metrics
            color_val = pred['derived'].get(param_color, pred['derived'].get('thickness_nm', 0))
            
            data.append(go.Scatter3d(
                x=t, y=[y_val]*len(t), z=th, mode='lines',
                line=dict(color=color_val, colorscale=cmap, width=5),
                name=f"{label[:15]}: {param_y}={y_val:.2f}",
                hovertemplate=f'<b>{label}</b><br>Time: %{{x:.2e}}s<br>Thickness: %{{z:.2f}}nm<br>{param_color}: {color_val:.3f}<extra></extra>'
            ))
        ...
        fig.update_layout(
            ...
            title=f'3D Thickness Evolution (Color: {param_color})',
            coloraxis_colorscale=cmap,
            height=800
        )
        return fig
    
    @staticmethod
    def parameter_map(predictions, param_x, param_y, metric='thickness_nm', cmap='jet', plot_type='scatter'):
        """
        ENHANCED: 2D contour/interpolated scatter of metric vs two parameters.
        Supports scatter, contour, and density contour plot types.
        """
        df = pd.DataFrame([
            {
                param_x: p['target_params'].get(param_x, np.nan),
                param_y: p['target_params'].get(param_y, np.nan),
                'metric': p['derived'].get(metric, p['derived'].get('thickness_nm', 0)),
        ...
        if plot_type == 'contour' and len(df) >= 10:
            # Try contour plot for denser data
            try:
                fig = px.density_contour(df, x=param_x, y=param_y, z='metric', 
                                        color='metric', colorscale=cmap,
                                        title=f'{metric} Map: {param_x} vs {param_y}')
            except:
                fig = px.scatter(df, x=param_x, y=param_y, size='metric', 
                               color='metric', colorscale=cmap,
                               hover_data=['label'],
                               title=f'{metric} Map: {param_x} vs {param_y}')
        else:
    ...
    @staticmethod
    def enhanced_weight_sunburst(predictions, labels, cmap='inferno'):
        """
        ENHANCED: Sunburst chart with hierarchical breakdown and colormap support.
        Includes individual parameter weights under beta component.
        """
        ids = ['all']
        ...
        for i, (pred, label) in enumerate(zip(predictions, labels)):
            ...
            for comp, comp_label in zip(comp_names, comp_labels):
                ...
                # Add sub-components for beta (individual parameter weights)
                if comp == 'beta' and 'individual_params' in weights:
                    indiv = weights['individual_params']
                    for param_name in ['fc', 'rs', 'c_bulk']:
                        if param_name in indiv:
                            sub_id = f"{comp_id}_{param_name}"
                            ids.append(sub_id)
                            labels_all.append(f'β_{param_name}')
                            parents.append(comp_id)
                            param_vals = indiv[param_name]
                            if isinstance(param_vals, list) and len(param_vals) > 0:
                                sub_val = np.mean(param_vals)
                            else:
                                sub_val = 0
        ...
        fig = go.Figure(go.Sunburst(
            ...
            values=values,
            branchvalues='total',
            marker=dict(colorscale=cmap)
        ))
        
        fig.update_layout(
            title='Interactive Weight Sunburst',
            height=800,
...
def main():
    ...
    if st.session_state.temporal_manager:
        ...
        with tabs[1]:
            ...
            # ENHANCED: Better handling for zero thickness cases
            stats_cols = st.columns(4)
            with stats_cols[0]:
                final_th = th_arr[-1] if len(th_arr) > 0 else 0.0
                st.metric("Final Thickness", f"{final_th:.3f} nm")
            with stats_cols[1]:
                if len(th_arr) > 1 and len(t_arr) > 1 and (t_arr[1]-t_arr[0]) > 0:
                    init_rate = (th_arr[1]-th_arr[0])/(t_arr[1]-t_arr[0])
                else:
                    init_rate = 0.0
                st.metric("Initial Growth Rate", f"{init_rate:.3f} nm/s")
            with stats_cols[2]:
                if len(th_arr) > 1 and len(t_arr) > 1 and (t_arr[-1]-t_arr[0]) > 0:
            ...
            with stats_cols[3]:
                if len(th_arr) > 0 and th_arr[-1] > 0:
                    idx_50 = np.argmin(np.abs(th_arr - 0.5*th_arr[-1]))
                    time_50 = t_arr[idx_50]
                else:
                    time_50 = 0.0
                st.metric("Time to 50% thickness", f"{time_50:.3e} s")
            ...
            if not st.session_state.saved_predictions:
                ...
                if selected_indices:
                    ...
                    plot_type = st.radio("Plot type",
                                        ["Thickness Evolution", "3D Thickness Evolution", 
                                         "Parameter Map", "Radar Metrics", "Weight Sunburst"])
                    ...
                    if plot_type == "Thickness Evolution":
                        ...
                        # ENHANCED: 3D thickness with colorscale
                        param_y = st.selectbox("Y-axis parameter", ['fc', 'rs', 'c_bulk', 'L0_nm'])
                        param_color = st.selectbox("Color parameter", ['thickness_nm', 'growth_rate', 'entropy'])
                        cmap_3d = st.selectbox("Colormap", ALL_COLORMAPS, index=ALL_COLORMAPS.index('turbo') if 'turbo' in ALL_COLORMAPS else 0)
                        
                        fig = st.session_state.multi_visualizer.enhanced_thickness_3d(
                            selected_preds, selected_labels, 
                            param_y=param_y, param_color=param_color, cmap=cmap_3d
                        )
                        st.plotly_chart(fig, use_container_width=True)
                    
                    elif plot_type == "Parameter Map":
                        # ENHANCED: Functional parameter map with options
                        ...
                        map_cmap = st.selectbox("Colormap", ALL_COLORMAPS, index=ALL_COLORMAPS.index('jet') if 'jet' in ALL_COLORMAPS else 0)
                        map_plot_type = st.selectbox("Plot type", ['scatter', 'contour'])
                        
                        fig = st.session_state.multi_visualizer.parameter_map(
                            selected_preds, param_x, param_y, metric, 
                            cmap=map_cmap, plot_type=map_plot_type
                        )
                        if fig:
                            st.plotly_chart(fig, use_container_width=True)
                        else:
                        ...
                        # ENHANCED: Interactive sunburst with colormap
                        sunburst_cmap = st.selectbox("Colormap", ALL_COLORMAPS, index=ALL_COLORMAPS.index('inferno') if 'inferno' in ALL_COLORMAPS else 0)
                        fig = st.session_state.multi_visualizer.enhanced_weight_sunburst(
                            selected_preds, selected_labels, cmap=sunburst_cmap
                        )
                        st.plotly_chart(fig, use_container_width=True)
        ...
        **ENHANCED Visualizations:**
        • 3D Thickness Evolution with 50+ colormaps (jet, turbo, inferno, rainbow, etc.)
        • Functional Parameter Map with contour/scatter options
        • Interactive Weight Sunburst with hierarchical breakdown
        • Fixed thickness statistics with adjustable thresholds
        """)
