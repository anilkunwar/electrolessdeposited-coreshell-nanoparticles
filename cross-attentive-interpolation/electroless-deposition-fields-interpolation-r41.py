...
=============================================================
- Natural language interface (regex‑based, with optional GPT‑2 parsing)
...
# NEW IMPORTS FOR GPT-2
try:
    from transformers import GPT2Tokenizer, GPT2LMHeadModel, pipeline
    TRANSFORMERS_AVAILABLE = True
except ImportError:
    TRANSFORMERS_AVAILABLE = False
    st.warning("transformers not installed. GPT‑2 features will be disabled.")
...
class DepositionPhysics:
    ...
    @staticmethod
    def compute_radial_profile(field, L0, center_frac=0.5, n_bins=100, use_median=False):  # MODIFIED: added use_median
        H, W = field.shape
...
class NLParser:
    """
    Extract deposition parameters from natural language input using regex or GPT-2.
    """
    def __init__(self):
        self.defaults = {
            'fc': 0.18,
    ...
    # NEW: Improved GPT-2 based parsing with robust JSON extraction and fallback
    def parse_with_gpt2(self, text: str, gpt2_tokenizer, gpt2_model, regex_params=None, temperature=0.1, num_return_sequences=1) -> dict:
        """Use GPT-2 to extract parameters from natural language with robust JSON handling.
        Optionally pass regex_params to include in prompt for better context."""
        if not text:
            return self.defaults.copy()
        
        # Build a prompt with few-shot examples to guide GPT-2
        examples = """
Examples:
...
"""
        ...
        if regex_params is not None:
            # Provide regex extraction as a hint (but instruct to prioritize the text)
            regex_hint = f"\nPreliminary regex extraction (use as a hint, but prioritize the text): {json.dumps(regex_params, default=str)}"
        prompt = f"""
Extract deposition parameters from the following text as a JSON object. Use the default values if a parameter is not mentioned.
IMPORTANT: Ignore examples if they conflict with the provided text. Extract ONLY from the text.
...
"""
        try:
            # Encode and generate
            inputs = gpt2_tokenizer.encode(prompt, return_tensors='pt', truncation=True, max_length=512)
            ...
            # Use regex to find the first JSON object (brace-balanced)
            json_pattern = r'\{[^{}]*(?:\{[^{}]*\}[^{}]*)*\}'
                ...
                if regex_params is not None:
                    for key in ['L0_nm', 'fc', 'rs', 'c_bulk']:
                        if key in regex_params and key in params:
                            # if regex extracted a value and GPT-2's differs significantly, warn and use regex
                            if abs(params[key] - regex_params[key]) > 1e-3:
                                st.warning(f"GPT-2 {key}={params[key]} mismatches regex {regex_params[key]}. Using regex for this parameter.")
                                params[key] = regex_params[key]
                return params
            else:
                # Fallback: attempt to extract individual parameters using regex
                st.warning("GPT-2 did not produce a valid JSON. Falling back to regex extraction.")
                return self._fallback_extract(generated)
        except Exception as e:
            st.warning(f"GPT-2 parsing failed: {e}. Falling back to regex.")
            return self.parse(text)  # fallback to regex

    # NEW: Ensemble method – run multiple generations and combine by averaging/consensus
    def parse_with_gpt2_ensemble(self, text: str, gpt2_tokenizer, gpt2_model, n_runs=3, regex_params=None, temperature=0.2) -> dict:
        """Run GPT-2 multiple times and combine results (average for floats, mode for categorical)."""
        all_params = []
        for _ in range(n_runs):
            params = self.parse_with_gpt2(text, gpt2_tokenizer, gpt2_model, regex_params, temperature=temperature, num_return_sequences=1)
        ...
        for key in self.defaults:
            ...
            if isinstance(self.defaults[key], bool):
                ...
                # For simplicity, take most common if multiple non-None, else None
    ...
    # NEW: Hybrid parse – combines regex and GPT-2 with confidence
    def hybrid_parse(self, text: str, gpt2_tokenizer, gpt2_model, use_ensemble=False, ensemble_runs=3) -> dict:
        """Run regex first, then GPT-2 (optionally ensemble), and merge based on confidence."""
        # Step 1: regex
        ...
        # Step 2: GPT-2 if available
        if gpt2_tokenizer is not None and gpt2_model is not None:
            if use_ensemble:
                gpt_params = self.parse_with_gpt2_ensemble(text, gpt2_tokenizer, gpt2_model, n_runs=ensemble_runs, regex_params=regex_params)
            else:
                gpt_params = self.parse_with_gpt2(text, gpt2_tokenizer, gpt2_model, regex_params=regex_params, temperature=0.1)
            # Confidence for GPT-2: moderate if extracted, lower if default
            gpt_conf = {}
            for key in self.defaults:
                gpt_conf[key] = 0.7 if gpt_params[key] != self.defaults[key] else 0.3
        else:
            gpt_params = self.defaults.copy()
            gpt_conf = {k: 0.0 for k in self.defaults}
        
        # Step 3: Merge with confidence
        final_params = {}
        for key in self.defaults:
            if regex_conf[key] > gpt_conf[key]:
                final_params[key] = regex_params[key]
            elif gpt_conf[key] > regex_conf[key]:
                final_params[key] = gpt_params[key]
            else:
                # tie: prefer regex if it matched, else average if numeric
                if regex_conf[key] > 0:
                    final_params[key] = regex_params[key]
                elif isinstance(regex_params[key], (int, float)) and isinstance(gpt_params[key], (int, float)):
                    final_params[key] = (regex_params[key] + gpt_params[key]) / 2
                else:
    ...
    def _fallback_extract(self, generated: str) -> dict:
        """Fallback: use regex to extract parameters from GPT-2's output if JSON fails."""
...
class CompletionAnalyzer:
    ...
    def compute_completion(manager, target_params: Dict, 
        ...
        Uses median and a softer threshold to avoid averaging artifacts."""
    ...
    # NEW: Improved GPT-2 completeness inference with robust JSON extraction
    @staticmethod
    def gpt_infer_completeness(desc: str, gpt2_tokenizer, gpt2_model) -> Tuple[bool, Optional[float], Optional[float], str]:
        """Use GPT-2 to infer completeness from a textual description of shell metrics."""
        prompt = f"""
Analyze this shell formation data and infer if the shell is complete (yes/no), estimate completion time (seconds, if known), minimal thickness (nm), and provide a brief explanation.

Data: {desc}

Output only a JSON object with keys: "complete" (bool), "t_complete" (float or null), "dr_min" (float or null), "explanation" (str).
...
"""
        try:
            ...
            if match:
                ...
                return (
                    ...
                    result.get('t_complete', None),
                    result.get('dr_min', None),
                    result.get('explanation', 'No explanation provided.')
                )
            else:
                # Fallback: try to parse with regex for simple yes/no
                if 'complete' in generated.lower():
                    if 'true' in generated.lower() or 'yes' in generated.lower():
                        return True, None, None, "GPT-2: Inferred complete (keyword)"
                    else:
                        return False, None, None, "GPT-2: Inferred incomplete (keyword)"
                return False, None, None, "GPT-2: Could not parse output."
        except Exception as e:
            return False, None, None, f"GPT-2 inference failed: {e}"
    ...
    def generate_recommendations(params: dict, relevance: float, 
                                  ...
                                  gpt_explanation: Optional[str] = None) -> List[str]:
        suggestions = []
        if relevance < 0.5:
            suggestions.append(
        ...
        # Add GPT-2 explanation if available
        if gpt_explanation:
            suggestions.append(f"🧠 **GPT-2 insight**: {gpt_explanation}")
        if not suggestions:
            suggestions.append(
...
def initialize_session_state():
    defaults = {
        ...
        'designer_input': "",
        # NEW: GPT-2 model and tokenizer
        'gpt2_tokenizer': None,
        'gpt2_model': None,
        'gpt2_available': TRANSFORMERS_AVAILABLE,
    }
...
def load_gpt2_model():
    if not TRANSFORMERS_AVAILABLE:
        return None, None
    with st.spinner("Loading GPT-2 model for NLP enhancements..."):
...
def render_intelligent_designer_tab():
    ...
    with col_gpt1:
        use_gpt2_parse = st.checkbox("Use GPT-2 for parameter extraction (hybrid)", 
    ...
    if gpt2_available and (use_gpt2_parse or use_gpt2_complete):
        if st.session_state.gpt2_tokenizer is None:
            tokenizer, model = load_gpt2_model()
            st.session_state.gpt2_tokenizer = tokenizer
            st.session_state.gpt2_model = model
        if st.session_state.gpt2_tokenizer is None:
            st.warning("GPT-2 model could not be loaded. Please check transformers installation.")
    
    if run_design and user_input:
        with st.spinner("🔍 Parsing natural language input..."):
            parser = st.session_state.nlp_parser
            # MODIFIED: Use hybrid_parse if GPT-2 is enabled
            if use_gpt2_parse and st.session_state.gpt2_tokenizer is not None:
                target_design = parser.hybrid_parse(
                    user_input, 
                    st.session_state.gpt2_tokenizer, 
                    st.session_state.gpt2_model,
                    use_ensemble=use_gpt2_ensemble,
        ...
        with st.spinner("🔬 Analyzing shell formation..."):
            ...
            # Use updated completeness detection with median and softer threshold
            t_complete, dr_min, is_complete = analyzer.compute_completion(
                design_manager, target_design, tolerance=0.3, use_median=True, completeness_threshold=0.95
            )
        ...
        # --- NEW: Shell quality metrics ---
        # Get fields at the requested time (or default to last)
        target_time = target_design.get('time')
        if target_time is not None:
            t_norm_target = target_time / design_manager.get_time_real(1.0)
            t_norm_target = np.clip(t_norm_target, 0, 1)
        else:
            t_norm_target = 1.0
        fields_target = design_manager.get_fields(t_norm_target, use_interpolation=True)
        core_radius_nm = target_design.get('fc', 0.18) * target_design.get('L0_nm', 60.0) / 2
        shell_quality = analyzer.compute_shell_quality(
            fields_target['phi'], fields_target['psi'], core_radius_nm, target_design.get('L0_nm', 60.0)
        )
        ...
        # --- GPT-2 completeness inference (if enabled) ---
        gpt_explanation = None
        if use_gpt2_complete and st.session_state.gpt2_tokenizer is not None:
            with st.spinner("🤖 GPT-2 analyzing shell completeness..."):
                # Build a textual description of the shell
                r, prof = DepositionPhysics.compute_radial_profile(
                    DepositionPhysics.material_proxy(fields_target['phi'], fields_target['psi']),
                ...
                """
                gpt_complete, gpt_t, gpt_dr, gpt_explanation = analyzer.gpt_infer_completeness(
                    desc, st.session_state.gpt2_tokenizer, st.session_state.gpt2_model
                )
                st.markdown("#### 🤖 GPT-2 Completeness Insight")
                st.info(f"**GPT-2 says:** {'Complete' if gpt_complete else 'Incomplete'} | "
                        f"Est. completion time: {gpt_t if gpt_t else 'N/A'} s | "
                        f"Min thickness: {gpt_dr if gpt_dr else 'N/A'} nm")
                st.caption(f"Explanation: {gpt_explanation}")
        ...
        # === FIXED: Use full temporal interpolation (any time) ===
        ...
        # This is the key line that was missing – now it gets fields for any time
        fields_sel = design_manager.get_fields(selected_time_norm, use_interpolation=True)
        t_sel_norm = selected_time_norm
        # ========================================================
        ...
        recommendations = analyzer.generate_recommendations(
            target_design, relevance, t_complete, dr_min, is_complete, shell_quality, gpt_explanation
        )
...
def main():
    ...
    # The remaining tabs are only active if a temporal manager exists
    ...
    with tabs[4]:
        ...
        if mgr is None:
            ...
            # Get full interpolation result at current time
