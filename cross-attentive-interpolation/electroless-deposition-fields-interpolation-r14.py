...
def compute_errors(gt_field, interp_field):
    ...
    # SSIM requires same shape and reasonable data range
...
def main():
    ...
    if st.session_state.temporal_manager:
        ...
        # UPDATED TABS LIST WITH NEW GROUND TRUTH COMPARISON TAB
        ...
        with tabs[6]:
            ...
            if not st.session_state.solutions:
                ...
                # Step 1: Filter and select ground truth
                target_params = mgr.target_params
                close_solutions = []
                for idx, sol in enumerate(st.session_state.solutions):
                    sol_params = sol['params']
                    # Normalize and compute Euclidean distance in parameter space
                    deltas = [
                        abs(DepositionParameters.normalize(target_params.get('fc', 0.18), 'fc') -
                            DepositionParameters.normalize(sol_params.get('fc', 0.18), 'fc')),
                        abs(DepositionParameters.normalize(target_params.get('rs', 0.2), 'rs') -
                            DepositionParameters.normalize(sol_params.get('rs', 0.2), 'rs')),
                        abs(DepositionParameters.normalize(target_params.get('c_bulk', 0.5), 'c_bulk') -
                            DepositionParameters.normalize(sol_params.get('c_bulk', 0.5), 'c_bulk')),
                        abs(DepositionParameters.normalize(target_params.get('L0_nm', 20.0), 'L0_nm') -
                            DepositionParameters.normalize(sol_params.get('L0_nm', 20.0), 'L0_nm'))
                    ]
                    dist = np.sqrt(sum(d**2 for d in deltas))
                    if dist < 0.15:  # Adjustable threshold for "close" matches
                        close_solutions.append((idx, dist, sol))
                
                if not close_solutions:
                    st.info("No close-matching ground truth found (Δ < 0.15). Showing all loaded solutions.")
                    close_solutions = [(idx, 0.0, sol) for idx, sol in enumerate(st.session_state.solutions)]
                
                # Sort by closeness (lowest distance first)
                close_solutions.sort(key=lambda x: x[1])
                
                # Display selection dropdown
                gt_options = [
                    f"Source {s[0]} (Δ={s[1]:.3f}): fc={s[2]['params'].get('fc',0):.3f}, rs={s[2]['params'].get('rs',0):.3f}, "
                    f"c={s[2]['params'].get('c_bulk',0):.2f}, L0={s[2]['params'].get('L0_nm',20):.1f} nm"
                    for s in close_solutions
                ]
                selected_gt_label = st.selectbox("Select Ground Truth Simulation", gt_options, key='gt_selector')
                
                if selected_gt_label:
                    # Extract selected ground truth
                    gt_idx = int(selected_gt_label.split("Source ")[1].split(" ")[0])
                    gt_sol = st.session_state.solutions[gt_idx]
                    gt_params = gt_sol['params']
                    
                    # Step 2: Recompute interpolation using GT params for fair comparison
                    with st.spinner("Computing interpolation at ground truth parameters..."):
                        ...
                        if interp_res:
                            # Step 3: Get GT fields at current normalized time
                            # Create lightweight temp manager for GT (minimal caching)
                            gt_mgr = TemporalFieldManager(
                                st.session_state.interpolator, [gt_sol], gt_params,
                                n_key_frames=3, lru_size=1
                            )
                            gt_fields = gt_mgr.get_fields(st.session_state.current_time, use_interpolation=False)
                            
                            # Ensure shapes match (resize GT if needed)
                            ...
                            # Compute material proxies for both
                            ...
                            # Define fields to compare with appropriate colormaps
                            compare_fields = {
                            ...
                            # For each field: display side-by-side + diff map + error metrics
                            for field_title, (field_key, cmap) in compare_fields.items():
                                st.subheader(f"Field: {field_title}")
                                ...
                                with col_err1:
                                    st.metric("MSE", f"{errors['MSE']:.4e}", help="Mean Squared Error")
                                with col_err2:
                                    st.metric("MAE", f"{errors['MAE']:.4e}", help="Mean Absolute Error")
                                with col_err3:
                                    st.metric("Max Error", f"{errors['Max Error']:.4e}", help="Maximum absolute difference")
                                with col_err4:
                                    if np.isnan(errors['SSIM']):
                                        st.metric("SSIM", "N/A", help="Structural Similarity (computation failed)")
                                    else:
                                        st.metric("SSIM", f"{errors['SSIM']:.3f}", help="Structural Similarity Index (1.0 = identical)")
                                
                                # Auto-generated insights based on metrics
                                insights = []
                                if errors['SSIM'] > 0.95:
                                    insights.append("✅ Excellent structural similarity – interpolation captures global patterns accurately.")
                                elif errors['SSIM'] > 0.85:
                                    insights.append("🟡 Good structural similarity – minor deviations in fine features.")
                                else:
                                    insights.append("🔴 Moderate/low similarity – notable structural differences; check interface regions.")
                                
                                if errors['Max Error'] > 0.15 * (gt_data.max() - gt_data.min() + 1e-8):
                                    insights.append("⚠️ High max errors likely near sharp interfaces or boundaries (φ≈0.5, ψ≈0.5).")
                                else:
                                    insights.append("✅ Low max errors – consistent accuracy across domain.")
                                
                                if errors['MSE'] < 1e-4:
                                    insights.append("✅ Very low average error – interpolation is highly accurate in bulk regions.")
                                
                                st.info("\n".join(insights))
                                
                                # Create 3-column layout for visual comparison
                                col_plot1, col_plot2, col_plot3 = st.columns(3)
                                with col_plot1:
                                    st.markdown("**📦 Ground Truth**")
                                    fig_gt = st.session_state.visualizer.create_field_heatmap(
                                        gt_data, "Ground Truth", cmap_name=cmap,
                                        L0_nm=gt_params.get('L0_nm', 20.0),
                                        target_params=gt_params,
                                        time_real_s=gt_mgr.get_time_real(st.session_state.current_time),
                                        colorbar_label="Material" if field_key == 'material' else field_key
                                    )
                                    st.pyplot(fig_gt)
                                with col_plot2:
                                    st.markdown("**🔄 Interpolated**")
                                    fig_interp = st.session_state.visualizer.create_field_heatmap(
                                        interp_data, "Interpolated", cmap_name=cmap,
                                        L0_nm=gt_params.get('L0_nm', 20.0),
                                        target_params=gt_params,
                                        time_real_s=interp_res.get('time_real_s', 0),
                                        colorbar_label="Material" if field_key == 'material' else field_key
                                    )
                                    st.pyplot(fig_interp)
                                with col_plot3:
                                    st.markdown("**📊 Difference (GT − Interp)**")
                                    # Use diverging colormap centered at zero for difference maps
                                    fig_diff = st.session_state.visualizer.create_field_heatmap(
                                        diff, "Difference", cmap_name='RdBu_r',
                                        L0_nm=gt_params.get('L0_nm', 20.0),
                                        target_params=gt_params,
                                        time_real_s=interp_res.get('time_real_s', 0),
                                        colorbar_label="Error", vmin=-np.max(np.abs(diff)), vmax=np.max(np.abs(diff))
                                    )
                                    st.pyplot(fig_diff)
                            
                            # Cleanup temporary GT manager to free memory
                            del gt_mgr
                        else:
                            st.error("Failed to compute interpolation for comparison. Check source data integrity.")
