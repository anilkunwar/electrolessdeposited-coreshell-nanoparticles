...
JSON:"""
    ...
    def generate_recommendations(params: dict, relevance: float, 
                                  ...
                                  llm_explanation: Optional[str] = None) -> List[str]:
        suggestions = []
        if relevance < 0.5:
            suggestions.append(
...
# =============================================
# RENDER INTELLIGENT DESIGNER TAB (UPDATED)
# =============================================
def render_intelligent_designer_tab():
    ...
    if run_design and user_input:
        ...
        # --- LLM completeness inference (if enabled) ---
        ...
        if use_llm_complete and st.session_state.llm_tokenizer is not None:
            with st.spinner("🤖 LLM analyzing shell completeness..."):
                ...
                """
                # ------------------------------------------------------------------
                # Use the improved llm_infer_completeness method (modified above)
                # ------------------------------------------------------------------
                llm_complete, llm_t, llm_dr, llm_explanation = analyzer.llm_infer_completeness(
        ...
        recommendations = analyzer.generate_recommendations(
            target_design, relevance, t_complete, dr_min, is_complete, shell_quality, llm_explanation
        )
