...
class CoreShellInterpolator:
    ...
    def interpolate_fields(self, sources: List[Dict], target_params: Dict,
        ...
        # Also ensure length matches source_fields (should, but just in case)
        if len(final_weights) != len(source_fields):
            # If mismatch, pad/truncate (should not happen)
            ...
            if len(final_weights) > len(source_fields):
                ...
                final_weights = np.pad(final_weights, (0, len(source_fields)-len(final_weights)), 'constant', constant_values=0)
        # -------------------------------------------------------------------------------
...
def main():
    ...
    if st.session_state.temporal_manager:
        ...
        # Tabs including the new Ground Truth Comparison
        ...
        with tabs[6]:
            ...
            if not st.session_state.solutions:
                ...
                # Let user pick any solution from the loaded list
                sol_options = [f"Source {idx}: {sol['metadata']['filename']}" for idx, sol in enumerate(st.session_state.solutions)]
                selected_idx = st.selectbox("Select Ground Truth PKL file", range(len(sol_options)), format_func=lambda i: sol_options[i])
                gt_sol = st.session_state.solutions[selected_idx]
                gt_params = gt_sol['params']
                
                # Optional: show ground truth parameters
                with st.expander("Ground truth parameters"):
                    st.json(gt_params)
                
                # Check L0 mismatch warning
                target_L0 = target.get('L0_nm', 20.0)
                gt_L0 = gt_params.get('L0_nm', 20.0)
                L0_diff = abs(target_L0 - gt_L0)
                if L0_diff > 1.0:
                    st.warning(f"Domain size mismatch: target L0 = {target_L0} nm, ground truth L0 = {gt_L0} nm. "
                               f"The difference map will align the smaller domain by padding with zeros "
                               f"(see note below).")
                
                # Get interpolated fields from the main manager (sidebar target)
                interp_fields = mgr.get_fields(current_time_norm, use_interpolation=True)
                
                # Helper to extract fields from a single solution at a given normalized time
                def get_fields_from_solution(solution, time_norm, target_shape=(256,256)):
                    history = solution.get('history', [])
                    if not history:
                        return None
                    # Determine t_max from thickness_history or last snapshot
                    if solution.get('thickness_history'):
                        t_max = solution['thickness_history'][-1]['t_nd']
                    else:
                        t_max = history[-1]['t_nd']
                    t_target = time_norm * t_max
                    ...
                    # If we didn't interpolate, snap is defined
                    phi = snap['phi']
                    c   = snap['c']
                    psi = snap['psi']
                    # Ensure 2D
                    if phi.ndim == 3:
                        phi = phi[phi.shape[0]//2, :, :]
                        c   = c[c.shape[0]//2, :, :]
                ...
                gt_fields = get_fields_from_solution(gt_sol, current_time_norm, target_shape=(256,256))
                
                if gt_fields is None:
                    st.error("Could not extract fields from the selected ground truth solution.")
                else:
                    ...
                    for field_title, (field_key, cmap_choice) in compare_fields.items():
                        ...
                        if field_key == 'material':
                            ...
                            interp_data = interp_fields[field_key]
                            gt_data = gt_fields[field_key]
                        
                        # ---- Handle L0 mismatch for difference map ----
                        if L0_diff > 1.0:
                            # Determine which domain is larger
                            if target_L0 >= gt_L0:
                                larger_field = interp_data
                                smaller_field = gt_data
                                larger_L0 = target_L0
                                smaller_L0 = gt_L0
                            else:
                                larger_field = gt_data
                                smaller_field = interp_data
                                larger_L0 = gt_L0
                                smaller_L0 = target_L0
                            
                            # Compute crop size in pixels for the smaller domain to fit into the larger grid
                            crop_px = int(round((smaller_L0 / larger_L0) * 256))
                            crop_px = max(1, min(crop_px, 256))
                            
                            # Resize smaller field to crop_px x crop_px
                            small_resized = zoom(smaller_field, (crop_px/256, crop_px/256), order=1)
                            
                            # Create zero array of size 256x256
                            padded_smaller = np.zeros((256, 256))
                            
                            # Place resized field in the center
                            start = (256 - crop_px) // 2
                            end = start + crop_px
                            padded_smaller[start:end, start:end] = small_resized
                            
                            # Use larger field as is, and compute diff on full grid
                            diff = larger_field - padded_smaller
                            diff_L0 = larger_L0
                            note = f" (smaller domain padded with zeros, L0 = {diff_L0} nm)"
                        else:
                            diff = gt_data - interp_data
                            diff_L0 = target_L0  # they are equal, so either works
                            note = ""
                        
                        # Errors (computed on the raw fields, not padded)
                        ...
                        with col_plot1:
                            ...
                            # For material proxy, explicitly set colorbar_label="Material"
                            cbar_label = "Material" if field_key == 'material' else None
                            ...
                            st.pyplot(fig_gt)
                        with col_plot2:
                            st.markdown("**Interpolated (sidebar target)**")
                            fig_interp = st.session_state.visualizer.create_field_heatmap(
                                interp_data, "Interpolated", 
                                cmap_name=cmap_choice, 
                                L0_nm=target_L0,
                                target_params=target,
                                time_real_s=current_time_real,
                        ...
                        with col_plot3:
                            st.markdown(f"**Difference (GT - Interp){note}**")
                            # For difference, never use material colormap
