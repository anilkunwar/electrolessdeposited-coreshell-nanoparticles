...
def main():
    ...
    with st.sidebar:
        ...
        correlation_params = st.multiselect(
            ...
            list(PARAM_CATEGORIES["⚙️ Physics Parameters"]) + list(PARAM_CATEGORIES["🔬 Geometry Parameters"]),
            default=['c_bulk', 'core_radius_frac', 'k0_nd']
        )
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        with st.expander("🔍 Filter & Sort Options", expanded=False):
            ...
            with col_f3:
                sort_col = st.selectbox("Sort by", 
                                      [target] + [c for c in df.columns if 'metric' in c or c in ['c_bulk', 'L0_nm']],
                                      index=0)
    ...
    with tab2:
        ...
        with col_r2:
            ...
            if len(df) >= n_to_show:
                ...
                sample_df = df[['c_bulk', 'core_radius_frac', 'use_edl']].copy()
    ...
    with tab3:
        ...
        with col_s1:
            primary_dim = st.selectbox(
                ...
                [c for c in df.columns if df[c].nunique() <= 10 or df[c].dtype in ['float64', 'float32']],
                index=0 if 'c_bulk' in df.columns else None
            )
        
        with col_s2:
            secondary_dim = st.selectbox(
                "Secondary Dimension (Outer Ring)", 
                [c for c in df.columns if c != primary_dim and (df[c].nunique() <= 15 or df[c].dtype in ['float64', 'float32'])],
                index=1 if 'core_radius_frac' in df.columns and len(df.columns) > 1 else 0
            )
