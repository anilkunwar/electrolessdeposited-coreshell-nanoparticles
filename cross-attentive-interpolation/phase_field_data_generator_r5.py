...
# -*- coding: utf-8 -*-
"""
ELECTROLESS Ag — SINGLE SIMULATION + MATERIAL PROXY + PHASE STATISTICS
* Single‑run focus (all batch functionality removed)
* Enhanced post‑processing: material proxy = max(phi, psi) + psi (default display)
* Phase area/volume statistics: electrolyte, Ag, Cu from thresholded fields
...
# ------------------- PAGE CONFIG -------------------
st.set_page_config(page_title="Electroless Ag — Single Run + Proxy", layout="wide")
st.title("Electroless Ag — Single Simulation + Material Proxy + Phase Statistics")
...
# Single c_bulk input (replaces batch multiselect)
c_bulk = st.sidebar.number_input(
    "c_bulk (bulk concentration)",
    min_value=0.01, max_value=2.0, value=1.0, step=0.01,
    format="%.3f", help="Single bulk concentration value"
)
...
def run_simulation(c_bulk_val):
    ...
    dz = L / (Nz - 1) if mode != "2D (planar)" else 1.0  # dummy for 2D
    center = L / 2.0
...
if run_button:
    with st.spinner(f"Running simulation for c_bulk = {c_bulk}..."):
        c, s, d, t, co, save_data = run_simulation(c_bulk)
        st.session_state.history[c] = {"snaps": s, "diag": d, "thick": t, "coords": co, "pkl": save_data}
        st.session_state.selected_c = c
        st.success("Simulation completed!")

# ------------------- PLAYBACK + PKL DOWNLOAD + PHASE STATISTICS -------------------
if st.session_state.history:
    st.header("Simulation Results")

    # Only one run in history, but we keep selection for consistency
    if len(st.session_state.history) == 1:
        selected_c = list(st.session_state.history.keys())[0]
    ...
    # Playback controls
    if snaps:
        st.subheader("Playback & Phase Analysis")
        frame_idx = st.slider("Frame", 0, len(snaps)-1, len(snaps)-1,
                             help="Select simulation frame to display")

        t_nd, phi, c_field, psi = snaps[frame_idx]
        t_real_ms = t_nd * tau0 * 1e3

        # Compute material proxy (max(phi, psi) + psi)
        proxy = np.maximum(phi, psi) + psi  # ranges 0-2
        ...
        # Display phase statistics as metrics
        st.markdown("**Phase Areas / Volumes**")
        if mode == "2D (planar)":
            cols = st.columns(3)
            with cols[0]:
        ...
        # Visualization: two side-by-side plots (phase field and material proxy)
        fig_playback, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 5))

        # Left: Phase field φ
        if mode == "2D (planar)":
            im1 = ax1.imshow(phi.T, cmap=cmap_choice, origin='lower',
                           extent=[0, domain_multiplier, 0, domain_multiplier])
            ax1.set_title(f"Phase field φ @ t = {t_real_ms:.1f} ms")
            ax1.set_xlabel("x (nd)")
            ax1.set_ylabel("y (nd)")
        else:
            mid_z = phi.shape[2] // 2
        ...
        # Right: Material proxy (default)
        if mode == "2D (planar)":
            im2 = ax2.imshow(proxy.T, cmap='viridis', origin='lower',
                           extent=[0, domain_multiplier, 0, domain_multiplier],
                           vmin=0, vmax=2)
            ax2.set_title(f"Material proxy (max(φ,ψ)+ψ) @ t = {t_real_ms:.1f} ms")
            ax2.set_xlabel("x (nd)")
            ax2.set_ylabel("y (nd)")
        else:
            mid_z = proxy.shape[2] // 2
            im2 = ax2.imshow(proxy[:, :, mid_z].T, cmap='viridis', origin='lower',
                           extent=[0, domain_multiplier, 0, domain_multiplier],
                           vmin=0, vmax=2)
            ax2.set_title(f"Material proxy (z-slice {mid_z}) @ t = {t_real_ms:.1f} ms")
            ax2.set_xlabel("x (nd)")
            ax2.set_ylabel("y (nd)")
        cbar2 = plt.colorbar(im2, ax=ax2, label="Proxy value", ticks=[0, 1, 2])
        cbar2.ax.set_yticklabels(['Electrolyte', 'Ag', 'Cu'])
        ...
        with col1:
            ...
            plt.close()
            st.download_button(
                label="📷 Download current φ as PNG",
                data=buf.getvalue(),
        ...
        with col2:
            ...
            plt.figure(figsize=(8, 6))
            if mode == "2D (planar)":
                plt.imshow(proxy.T, cmap='viridis', origin='lower',
                          extent=[0, domain_multiplier, 0, domain_multiplier],
                          vmin=0, vmax=2)
            else:
                mid_z = proxy.shape[2] // 2
                plt.imshow(proxy[:, :, mid_z].T, cmap='viridis', origin='lower',
                          extent=[0, domain_multiplier, 0, domain_multiplier],
                          vmin=0, vmax=2)
            plt.title(f"Material proxy @ t = {t_real_ms:.1f} ms")
            ...
            plt.close()
            st.download_button(
                label="📷 Download proxy as PNG",
                data=buf.getvalue(),
                file_name=f"Ag_proxy_{frame_idx}_t{t_real_ms:.1f}ms.png",
                mime="image/png"
            )
        ...
        with col3:
            ...
            if GIF_AVAILABLE and len(snaps) > 1:
                if st.button("🎬 Create GIF animation (proxy)"):
                    with st.spinner("Creating GIF..."):
                        ...
                        def animate_frame(i):
                            ...
                            proxy_frame = np.maximum(phi_frame, psi_frame) + psi_frame
                            t_ms = t_nd * tau0 * 1e3
                            if mode == "2D (planar)":
                                im = ax.imshow(proxy_frame.T, cmap='viridis', origin='lower',
                                             extent=[0, domain_multiplier, 0, domain_multiplier],
                                             vmin=0, vmax=2)
                            else:
                                mid_z = proxy_frame.shape[2] // 2
                                im = ax.imshow(proxy_frame[:, :, mid_z].T, cmap='viridis', origin='lower',
                                             extent=[0, domain_multiplier, 0, domain_multiplier],
                                             vmin=0, vmax=2)
                        ...
                        plt.close()
                        st.download_button(
                            label="📥 Download GIF (proxy)",
                            data=gif_buf.getvalue(),
                            file_name=f"Ag_proxy_c{selected_c:.3f}.gif",
                            mime="image/gif"
                        )
            else:
                st.info("GIF export requires imageio and multiple frames")
