# --------------------------------------------------------------
# ELECTROLESS Ag – THERMODYNAMICALLY CONSISTENT FREE ENERGY
# --------------------------------------------------------------
...
# -------------------- 3. Free-energy derivative --------------------
@njit(parallel=True, fastmath=True)
def free_energy_derivative(phi, psi, a_index, beta, h, W, kBT, eps):
    """
    Thermodynamically consistent f'(phi,psi).
    All arrays must be float32.
    """
    ny, nx = phi.shape
    f_prime = np.zeros_like(phi)

    # polynomial part: W * (2phi-1)*(1-2phi+2phi^2)
    z = 2.0*phi - 1.0
    poly = (1.0 - 2.0*phi + 2.0*phi*phi)          # = 2*z^2
    poly_deriv = W * z * poly

    # harmonic term inside template
    harm = 2.0 * beta * (phi - h)

    # logarithmic barrier (protected)
    phi_cl = np.where(phi < eps, eps, phi)
    phi_cl = np.where(phi_cl > 1.0-eps, 1.0-eps, phi_cl)
    log_phi = np.log(phi_cl)
    log_omphi = np.log(1.0 - phi_cl)
    entropic = kBT * (log_phi - log_omphi)
...
def run_simulation_live(
    ...
    M_Ag, rho_Ag, beta, a_index, h, W, kBT, eps_log,
    ...
    surface = np.exp(-np.abs(dist)/ratio_decay_len)
    ...
    for step in range(n_steps+1):
        ...
        # ----- NEW free-energy derivative -----
        f_prime = free_energy_derivative(phi, psi, a_index, beta, h, W, kBT, eps_log)
...
# -------------------- 6. UI --------------------
st.title("Electroless Ag – Thermodynamically Consistent Free Energy")
st.markdown("**Logarithmic + polynomial barrier, no clipping, stable shell growth.**")
...
st.sidebar.header("Free-Energy Parameters")
W = st.sidebar.number_input("W (barrier height)", 1.0, 50.0, 10.0, 0.5)
kBT = st.sidebar.number_input("k_B T (eV)", 1e-3, 0.1, 0.025, 1e-3,
                              help="~0.025 eV at room temperature")
eps_log = st.sidebar.number_input("ϵ_log", 1e-6, 1e-1, 1e-3, 1e-4)
...
if run_col.button("Run Simulation"):
    ...
    if cached:
        st.session_state.results = cached; status.success("Loaded cached run")
    else:
        ...
        results = run_simulation_live(
            ...
            0.10787, 10500, beta, a_index, h, W, kBT, eps_log,
            psi, AgNH3_conc, Cu_ion_conc, eta_chem,
            ratio_top_factor, ratio_surface_factor, ratio_decay_len,
            save_every, ui
        )
...
if st.session_state.results:
    ...
    time_idx = st.slider("Time step", 0, len(t_hist)-1, len(t_hist)//2,
                         format="t = %.2f s")
    var = st.selectbox("Variable", ["ϕ – Ag phase", "c – concentration",
                                   "ψ – template", "ratio field"])
    data = phi_hist[time_idx] if var.startswith("ϕ") else \
           c_hist[time_idx] if var.startswith("c") else \
           psi if var.startswith("ψ") else ratio_field
    ...
    mid = Nx//2
    fig2, ax = plt.subplots()
    ax.plot(y, data[mid,:]); ax.set_xlabel('y (cm)')
    ax.set_title(f'{var.split()[0]} at x=Lx/2, t={t_hist[time_idx]:.2f}s')
    ax.grid(True); st.pyplot(fig2); plt.close(fig2)

    # VTR export (single timestep)
