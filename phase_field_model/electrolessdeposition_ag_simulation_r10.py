...
--------------------------------
2D/3D Phase-Field Electroless Ag Deposition Simulator
- Molar ratio [Ag]/[Cu] controls c_bulk
- Shell thickness increases with ratio
- Real time (up to 150 s), real length (nm)
- Multi-ratio sweep with plots
"""
...
L0 = st.sidebar.number_input("L₀ (nm)", 1.0, 1e6, 20.0, 1.0,
                             help="Reference length (e.g., Cu core diameter)")
...
conc_mode = st.sidebar.selectbox("Mode", ["Fixed c_bulk", "Molar Ratio [Ag]/[Cu]"])

if conc_mode == "Fixed c_bulk":
    c_bulk_nd = st.sidebar.slider("c_bulk (non-dim)", 0.1, 10.0, 2.0, 0.1)
    molar_ratios = [c_bulk_nd]
else:
    st.sidebar.markdown("**Active ratios [Ag]/[Cu]:**")
    molar_ratios = st.sidebar.multiselect(
        "Select ratios", [0.2, 0.25, 0.33, 0.5, 1.0, 2.0],
        default=[0.2, 0.5, 1.0]
    )
    if not molar_ratios:
        molar_ratios = [1.0]
        st.sidebar.warning("Select at least one ratio.")
    c_bulk_nd = molar_ratios[0]  # for single run
...
phi_threshold = st.sidebar.slider("Phi threshold (shell)", 0.1, 0.9, 0.5, 0.05)
...
def real_to_nd(x): return x / L0
def scale_time(t): return t * tau0
def scale_D(D): return D * (L0**2 / tau0)
def scale_M(M): return M * (L0**3 / (E0 * tau0))
def scale_k0(k0): return k0 * (L0 / tau0)
...
def compute_thickness(phi, psi, coords, mode="2D", thresh=0.5):
    ...
    if np.any(shell):
        ...
        r_core = core_radius_frac
        thick_nd = r_max - r_core
...
def grad_mag_2d(u, dx):
    ux = (u[:,2:] - u[:,:-2]) / (2*dx); ux = np.pad(ux, ((0,0),(1,1)), mode='edge')
    uy = (u[2:,:] - u[:-2,:]) / (2*dx); uy = np.pad(uy, ((1,1),(0,0)), mode='edge')
...
def run_sim(params):
    ...
    r_outer = r_core * (1 + params['shell_thickness_frac'])
...
if run_multi and conc_mode == "Molar Ratio [Ag]/[Cu]":
    results = {}
    prog = st.progress(0)
    for i, r in enumerate(molar_ratios):
        st.write(f"Running [Ag]/[Cu] = {r}")
        params = {
            'Nx': Nx, 'Ny': Ny, 'Nz': Nz, 'dt': dt_nd, 'n_steps': n_steps, 'save_every': save_every,
            'gamma': gamma_nd, 'beta': beta_nd, 'k0': k0_nd, 'M': M_nd, 'D': D_nd, 'alpha': alpha_nd,
            'c_bulk': r, 'core_radius_frac': core_radius_frac, 'shell_thickness_frac': shell_thickness_frac,
            'bc_type': bc_type, 'phi_threshold': phi_threshold
        }
        snapshots, diags, thicks, coords = run_sim(params)
        results[r] = {'thicks': thicks}
...
if st.session_state.get('snapshots'):
    ...
    ax2.set_xlabel("Time (s)"); ax2.set_ylabel("Shell Thickness (nm)")
...
# ------------------- Export -------------------
if export_vtu and st.session_state.get('snapshots') and MESHIO_AVAILABLE:
    with tempfile.TemporaryDirectory() as tmp:
        vtus = []
        for i, (t, phi, c, psi) in enumerate(st.session_state.snapshots):
            points = np.column_stack([nd_to_real(np.ravel(X)) for X in np.meshgrid(*st.session_state.coords, indexing='ij')])
            data = {"phi": phi.ravel(), "c": c.ravel(), "psi": psi.ravel()}
            fname = f"{tmp}/frame_{i:04d}.vtu"
            meshio.write_points_cells(fname, points, [], point_data=data)
            vtus.append(fname)
        # ZIP
        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as z:
            for f in vtus: z.write(f, os.path.basename(f))
        buf.seek(0)
        st.download_button("Download VTU ZIP", buf, "frames.zip")

st.info("Tip: Higher [Ag]/[Cu] → thicker shell. Use **Run All Ratios** to compare.")
