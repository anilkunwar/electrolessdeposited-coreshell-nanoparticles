...
# -*- coding: utf-8 -*-
"""
Electroless Ag — FULL GPU-ACCELERATED SIMULATOR (CuPy)
All features:
  • Bulk vs. gradient norms
  • Material + potential proxy
  • Multi-run history + combined plots
  • VTU/PVD/ZIP export (CPU fallback)
  • GIF animation
  • PNG snapshot, CSV, autoplay
  • BATCH SIMULATION (parallel CPU + GPU per run)
  • FULL CuPy GPU acceleration
"""
...
try:
    ...
    GPU_AVAILABLE = True
    st.sidebar.success("CuPy GPU detected")
except Exception as e:
    import numpy as cp
    from numpy.fft import fft2, ifft2
    GPU_AVAILABLE = False
    st.sidebar.warning(f"CuPy not available: {e}\nFalling back to NumPy (CPU)")
...
# ------------------- PAGE -------------------
st.set_page_config(page_title="Electroless Ag — GPU Simulator", layout="wide")
st.title("Electroless Ag — GPU-Accelerated (CuPy)")
...
if mode.startswith("2D"):
    Nx = st.sidebar.slider("Nx", 64, 1024, 256, 32)
    Ny = st.sidebar.slider("Ny", 64, 1024, 256, 32)
    Nz = 1
else:
    Nx = st.sidebar.slider("Nx", 32, 128, 64, 8)
    Ny = Nx
    Nz = st.sidebar.slider("Nz", 32, 128, 64, 8)

dt_nd = st.sidebar.number_input("dt (nd)", 1e-6, 1e-2, 1e-4, format="%.6f")
n_steps = st.sidebar.slider("Steps", 50, 4000, 1000, 50)
...
use_fft = st.sidebar.checkbox("Use FFT Laplacian (GPU only)", GPU_AVAILABLE)
...
run_batch_button = st.sidebar.button("Run BATCH (GPU)")
...
if GPU_AVAILABLE and use_fft:
    def fft_laplacian(u):
        ...
        u_hat = cpfft.fftn(u)
        lap_hat = -K2 * u_hat
        return cpfft.ifftn(lap_hat).real
else:
    def fft_laplacian(u):
...
def laplacian(u, dx):
    if GPU_AVAILABLE and use_fft:
        return fft_laplacian(u)
    return lap2d_fd(u, dx) if u.ndim == 2 else lap3d_fd(u, dx)

# ------------------- THICKNESS -------------------
def compute_thickness(phi, psi, coords, core_frac, thresh):
    x, y, z = coords
    X, Y, Z = cp.meshgrid(cp.array(x), cp.array(y), cp.array(z), indexing='ij')
    dist = cp.sqrt((X-0.5)**2 + (Y-0.5)**2 + (Z-0.5)**2)
    mask = (phi > thresh) & (psi < 0.5)
    return float(cp.max(dist[mask]) - core_frac) if cp.any(mask) else 0.0
...
def run_simulation(c_bulk_val):
    ...
    if mode == "2D (planar)":
        ...
        dist = cp.sqrt((X-0.5)**2 + (Y-0.5)**2)
        coords = (to_cpu(x), to_cpu(y))
    else:
        X, Y, Z = cp.meshgrid(x, x, x, indexing='ij')
        dist = cp.sqrt((X-0.5)**2 + (Y-0.5)**2 + (Z-0.5)**2)
        coords = (to_cpu(x), to_cpu(x), to_cpu(x))
    ...
    for step in range(n_steps + 1):
        ...
        if step % save_every == 0 or step == n_steps:
            th_nd = compute_thickness(phi, psi, coords, core_radius_frac, phi_threshold)
            max_th = max(max_th, th_nd)
            c_mean, c_max = float(cp.mean(c)), float(cp.max(c))
            total_ag = float(cp.sum(i_loc) * dt_nd)

            # Save to CPU only when needed
            snapshots.append((t, to_cpu(phi), to_cpu(c), to_cpu(psi)))
...
if run_batch_button and c_bulk_list:
    with st.spinner(f"Running BATCH of {len(c_bulk_list)} simulations on GPU..."):
        results = []
        with ProcessPoolExecutor() as executor:
            ...
            for future in as_completed(future_to_c):
                try:
                    ...
                    st.error(f"GPU sim failed: {e}")
        for res in results:
            c_bulk_val, snaps, diag, thick, coords = res
            st.session_state.history[c_bulk_val] = {"snaps": snaps, "diag": diag, "thick": thick, "coords": coords}
        if results:
            st.session_state.selected_c = results[0][0]
        st.success(f"GPU Batch completed: {len(results)} runs")

# ------------------- SINGLE RUN -------------------
if run_single_button and selected_labels:
    c_bulk_val = c_options[selected_labels[-1]]
    with st.spinner(f"Running SINGLE c = {c_bulk_val} on GPU..."):
        t0 = time.time()
        c_bulk_val, snaps, diag, thick, coords = run_simulation(c_bulk_val)
        st.session_state.history[c_bulk_val] = {"snaps": snaps, "diag": diag, "thick": thick, "coords": coords}
        st.session_state.selected_c = c_bulk_val
        st.success(f"GPU run done in {time.time()-t0:.2f}s")

# ------------------- COMBINED PLOTS -------------------
if len(st.session_state.history) > 1:
    st.header("Batch Comparison (GPU)")
...
if st.session_state.history:
    ...
    if st.sidebar.button("Export VTU/PVD/ZIP") and MESHIO_AVAILABLE:
        with st.spinner("Exporting VTU/PVD (CPU)..."):
            ...
            zipbuf.seek(0)
            st.download_button("Download VTU/PVD ZIP", zipbuf.read(), f"frames_gpu_{datetime.now():%Y%m%d_%H%M%S}.zip", "application/zip")
    ...
    if st.sidebar.button("Generate GIF") and GIF_AVAILABLE:
        with st.spinner("Generating GIF..."):
            ...
            gif_buf.seek(0)
            st.download_button("Download GIF", gif_buf.read(), f"gpu_animation_{field.split()[0]}.gif", "image/gif")
    ...
    if st.sidebar.button("Download PNG Snapshot"):
        ...
        buf.seek(0)
        st.download_button("Download PNG", buf.read(), f"gpu_snapshot_{field.split()[0]}.png", "image/png")
    ...
    st.info("Run a simulation (GPU) to see results.")
