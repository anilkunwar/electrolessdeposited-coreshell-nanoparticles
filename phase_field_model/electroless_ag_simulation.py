...
def _run_simulation_fast(_run_id,
    ...
    return (x, y,
            ...
            np.zeros_like(phi_hist), np.array(t_hist), psi)
...
dt = st.sidebar.number_input("Δt (s)", 5e-7, 5e-5, 2e-6, 1e-7)
...
save_every = st.sidebar.number_input("Save every N steps", 1, 50, 10, 1)
...
if st.session_state.results:
    ...
    idx = st.slider("Frame", 0, len(t_hist) - 1, 0, format="t = %.2f s")
    t = t_hist[idx]
    phi = phi_hist[idx]
    c = c_hist[idx]

    cs = st.selectbox("Color scale", ["Viridis", "Plasma", "Magma", "Jet", "Turbo"])

    for arr, name in [(phi, "φ (Ag)"), (c, "c (mol/cm³)"), (psi, "ψ (Cu core)")]:
        fig = go.Figure(go.Contour(z=arr, x=x, y=y, colorscale=cs))
        fig.update_layout(title=f"{name} @ t = {t:.2f}s",
    ...
    # ---- line plot ----
    mid = Nx // 2
    for arr, lab, col in [(phi[:, mid], "φ", "steelblue"),
                          (c[:, mid], "c", "crimson")]:
        fig, ax = plt.subplots()
        ax.plot(y, arr, color=col, label=lab)
        ax.set_xlabel("y (cm)"); ax.set_ylabel(lab)
        ax.set_title(f"{lab} @ x = Lx/2")
        ax.legend(); ax.grid(True)
        st.pyplot(fig); plt.close(fig)

    # ---- VTR download (cached) ----
    @st.cache_data
    def _vtr_bytes(_t, _phi, _c, _psi, _x, _y):
    ...
    vtr = _vtr_bytes(t, phi, c, psi, x, y)
    st.download_button(
        f"Download VTR (t = {t:.2f}s)",
        vtr, f"ag_t{t:.2f}.vtr", "application/octet-stream")

    # ---- ZIP all frames ----
    if st.button("Download ALL VTRs (ZIP)"):
        @st.cache_data
        def _zip_all(_phi_hist, _c_hist, _psi, _t_hist, _x, _y):
            bio = BytesIO()
            with zipfile.ZipFile(bio, "w", zipfile.ZIP_DEFLATED) as z:
                for i, tt in enumerate(_t_hist):
                    ...
                    grid.save(tmp.name)
                    z.write(tmp.name, f"ag_t{tt:.2f}.vtr")
                    tmp.close()
                    Path(tmp.name).unlink()
            return bio.getvalue()

        zip_data = _zip_all(phi_hist, c_hist, psi, t_hist, x, y)
        st.download_button("Download ZIP", zip_data,
