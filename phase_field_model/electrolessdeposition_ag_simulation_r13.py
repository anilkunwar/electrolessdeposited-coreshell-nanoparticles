...
# Try to import plotly for optional interactive plotting
try:
...
# concentration input method for batch
...
# fallback single concentration param (for single-run UI)
if study_mode == "Single concentration (1:1)":
    c_bulk_nd = st.sidebar.slider("c_bulk ([Ag]/[Cu])", 0.1, 10.0, 1.0, 0.1)
else:
    # for batch runs initial value shown if user wants to run single later
    c_bulk_nd = 1.0
...
def run_simulation_2d(params):
    ...
    for step in range(n_steps+1):
        ...
        if has_factor:
            ...
            # apply factorization for stability (only as a formality here)
...
def run_multiple_simulations(params_base, concentrations):
    ...
    for i, conc in enumerate(concentrations):
        ...
        # run selected mode only (2D or 3D)
...
# ------------------- helper: parse concentrations from UI -------------------
def get_concentrations_from_ui():
    if conc_input_mode.startswith("Manual"):
        ...
        # slider mode: produce linspace between start and end with steps
        start = slider_start
        end = slider_end
        steps = slider_steps
        if steps <= 1:
...
# ------------------- run single simulation (button) -------------------
if run_single_button:
    t0 = time.time()
    st.info("Running single simulation …")
    # ensure params_base updated with current growth model and c_bulk
...
if run_batch_button:
    ...
    # sanitize and unique-sort concentrations
...
if st.session_state.multiple_results:
    ...
    for i, conc in enumerate(concentrations_sorted):
        ...
        th_nm = [th*1e9 for _, _, th in data]
        label = f"c={conc:.4g}"
    ...
    if use_plotly and PLOTLY_AVAILABLE:
        ...
        if not df_plot.empty:
            figp = px.line(df_plot, x="t_s", y="thickness_nm", color="c_bulk",
                           labels={"t_s": "Time (s)", "thickness_nm": "Thickness (nm)", "c_bulk": "c_bulk"},
                           title=f"Interactive Thickness vs Time — {mode}")
            st.plotly_chart(figp, use_container_width=True)
        else:
            st.info("Not enough data for interactive plot.")

    # Summary table of final thickness per concentration
    summary_rows = []
    for conc in concentrations_sorted:
        data = multiple_results[conc]['thickness']
        if not data:
        ...
        summary_rows.append({"c_bulk": conc, "Thickness_nd": final_nd, "Thickness_nm": final_nm})
    if summary_rows:
        df_summary = pd.DataFrame(summary_rows).sort_values("c_bulk", ascending=False)
        st.subheader("Final Thickness Summary")
        st.dataframe(df_summary.style.format({"c_bulk": "{:.6g}", "Thickness_nm": "{:.2f}", "Thickness_nd": "{:.4f}"}))
        csv_bytes = df_summary.to_csv(index=False).encode()
        st.download_button("Download final thicknesses (CSV)", csv_bytes, file_name=f"thickness_summary_{datetime.now():%Y%m%d_%H%M%S}.csv", mime="text/csv")
