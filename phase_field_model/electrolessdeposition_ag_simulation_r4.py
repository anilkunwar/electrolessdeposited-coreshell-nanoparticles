...
def run_simulation_2d(params):
    ...
    for step in range(n_steps + 1):
        ...
        if has_factor:
            phi = lu(phi_temp.reshape(-1)).reshape(phi.shape)
        else:
...
if st.session_state.snapshots:
    ...
    potential = -alpha * c_view                     # proxy for the driving term
    ...
    st.download_button("Download diagnostics CSV",
                       csv_data,
                       file_name=f"diagnostics_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                       mime="text/csv")

    # PNG snapshot of current frame (original field)
    img_buf = io.BytesIO()
    if mode.startswith("2D"):
    ...
    if export_vtu_button:
        if not MESHIO_AVAILABLE:
            st.error("meshio not installed; cannot export VTU/PVD. Install `meshio`.")
        else:
            ...
            for idx, (tframe, phi_s, c_s, psi_s) in enumerate(snapshots):
                ...
                # ---- add material field to point data ----
                mat_s = build_material(phi_s, psi_s, material_method)
                point_data = {
...
164                    "material": mat_s.ravel().astype(np.float32)
            ...
            st.download_button("Download VTU/PVD ZIP",
                               zipbuf.read(),
                               file_name=f"frames_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip",
    ...
    if MESHIO_AVAILABLE:
        ...
        if st.button("Download selected frame as VTU"):
            ...
            buf.seek(0)
            st.download_button("Download VTU (selected frame)",
                               buf.read(),
                               file_name=f"frame_{int(sel_frame_for_vtu):04d}.vtu",
